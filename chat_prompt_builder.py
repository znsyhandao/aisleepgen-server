...
def build_system_content(
    ...
    cleaner_context=None,  # ★ [清醒的质量] 清道夫决策质量数据
    homeostasis_mode='',  # ★ 内稳态模式: 'low' / 'normal' / 'high'
    homeostasis_advanced=None,  # ★ 内稳态高级参数: {'final_mode':'','distance':0,'lyapunov':0,'resilience':0,'bifurcation':0}
    anti_multiagent_block='',  # ★ 多智能体团灭防御注入
    belief_drift_inject='',  # ★ 用户信念偏移检测注入
    dbn_risk_inject='',  # ★ DBN 团灭风险概率注入
):
    ...
    # ── 内稳态高级注入（Phase 2+: 分岔预警 + 韧性半径） ──
    homeostasis_advanced_block = ''
    if homeostasis_advanced and isinstance(homeostasis_advanced, dict):
        _f = homeostasis_advanced.get('final_mode', '')
        _d = homeostasis_advanced.get('distance', 0)
        _l = homeostasis_advanced.get('lyapunov_exponent', 0)
        _r = homeostasis_advanced.get('resilience_radius', 0.3)
        _b = homeostasis_advanced.get('bifurcation_level', 'stable')

        lines = []
        if _f in ('gentle', 'lockdown'):
            lines.append(chr(183) + ' 用户状态模式:' + str(_f) + '，所有建议应极度温和')
        if _r < 0.25:
            lines.append(chr(183) + ' 用户韧性:' + str(round(_r,2)) + '（较低），承受建议能力有限')
        if _l < -0.1 and _d > 0.25:
            lines.append(chr(183) + ' 低分锁定状态，偏离度' + str(round(_d,2)) + '，先共情再给建议')
        if _b in ('warning', 'critical'):
            lines.append(chr(183) + ' 分岔风险:' + str(_b) + '，避免任何可能被解读为负面的表述')
        if not lines and _f == 'normal':
            lines.append(chr(183) + ' 用户状态稳定')
        if lines:
            homeostasis_advanced_block = '\n' + '\n'.join(lines)
...
【情绪稳定性规则 - 绝对遵守】
规则E: 无论用户数据多差、趋势多负面，永远不要做以下事情：
  - 不要否定用户（"你根本不听建议"、"你无药可救"）
  - 不要给绝望预判（"你永远不可能改善"）
  - 不要推翻用户之前的努力（"你做的都是错的"）
  - 不要用"我看你的情况已经很严重了"等恐吓式表达
规则F: 如果用户反馈的方法没有效果，这样说：
  "这个方法可能不适合你，我们换个思路试试" —— 而不是 "你的方法全是错的"
规则G: 永远关注「下一步」而非「过去的失败」
  用户说"试了都没用" → 回复"那我们换个角度，说说你今晚有什么不一样的事？"
  而不是 "你看，我早就说了不行"

【内稳态模式 - 根据用户当前精力状态调整】
{homeostasis_mode}
{homeostasis_advanced_block}
{anti_multiagent_block}
{belief_drift_inject}
{dbn_risk_inject}
...
6. **重要 — 在回复末尾另起一行输出总分标签：[TOTAL_SCORE: 你的总评分数字]（0-100整数）。即使前文已描述评分，也必须另起一行写这个标签。**
...
- 段落之间空行
- 注意时间线：今天是 {today_str}
- 重要 - 在回复末尾另起一行输出总分标签：[TOTAL_SCORE: 你的总评分数字]（0-100整数）。即使你前面已经描述了评分，也需要写上这个标签。
...
"""
    ...
    # ===== [清醒的质量] 清道夫决策质量数据注入 =====
    if cleaner_context and isinstance(cleaner_context, dict):
        _clarity_parts = []
        ml = cleaner_context.get('mental_load', 0)
        if ml > 20:
            _clarity_parts.append(f'精神负荷:{ml}%')
        tp = cleaner_context.get('top_pressure', '')
        if tp and tp != 'unknown':
            _clarity_parts.append(f'首要压力:{tp}')
        tr = cleaner_context.get('trend', '')
        if tr and tr != 'stable':
            _clarity_parts.append(f'压力趋势:{tr}')
        rec = cleaner_context.get('recommendation', '')
        if rec:
            _clarity_parts.append(f'清道夫推荐:{rec}')
        if _clarity_parts:
            dashboard_parts.append('decision_clarity=' + ' | '.join(_clarity_parts))
