...
# ===== 版本号 =====
VERSION = '2.9.0'
VERSION_TAG = 'user-feedback'  # user feedback loop: close the coach suggestion->action->feedback loop  # sleep coach: suggestion->action->feedback->optimize loop  # 推理缓存层：世界模型+API+Profile三缓存
CHANGES = [
    'v2.9.0 Coach feedback API: POST /api/coach-feedback',
    'v2.9.0 Feedback types: done / not_done / forgot',
    'v2.9.0 Done feedback: +15 weight to suggestion strategy',
    'v2.9.0 Not done feedback: -10 weight to suggestion strategy',
    'v2.9.0 Pending feedback detection: scheduler prompts user for yesterday feedback',
    'v2.9.0 Feedback stats: completion rate, feedback rate, strategy effectiveness',
    'v2.9.0 Full closed loop: suggestion -> action -> feedback -> optimize',
]
...
VERSION_TAG = 'user-feedback'  # user feedback loop: close the coach suggestion->action->feedback loop  # sleep coach: suggestion->action->feedback->optimize loop  # 睡眠教练：建议→执行→反馈→优化闭环
