# -*- coding: utf-8 -*-
"""
睡眠世界模型 v5.0 - 10专家交叉会诊 + 循证推理架构 + 拓扑分解
...
class ClinicalPsychologist:
    def analyze(self, data: Dict, peer_findings: Dict = None, evidence: list = None) -> Dict:
        ...
        # [v4.2] 拓扑身份：长期心理特征(h主导)
        topo_bias = {'phi': 0.15, 'psi': 0.15, 'h': 0.70}

        return {'score':round(score,2),'findings':findings,'risk_flags':risk_flags,'phq9_sim':round(base_phq,1),'gad7_sim':round(base_gad,1),'confidence':confidence,'specialty':'临床心理学','topo_bias':topo_bias}
...
class CognitiveBehavioralTherapist:
    def analyze(self, data: Dict, peer_findings: Dict = None, evidence: list = None) -> Dict:
        ...
        # [v4.2] 拓扑身份：可恢复行为(phi主导)
        topo_bias = {'phi': 0.70, 'psi': 0.15, 'h': 0.15}
        ...
        return {'score':round(score,2),'findings':findings,'risk_flags':risk_flags,
                ...
                'confidence':0.80,'specialty':'认知行为治疗(CBT-I)','topo_bias':topo_bias}
...
class SleepPhysician:
    def analyze(self, data: Dict, peer_findings: Dict = None, evidence: list = None) -> Dict:
        ...
        # [v4.2] 拓扑身份：生理筛查(phi+psi)
        topo_bias = {'phi': 0.45, 'psi': 0.35, 'h': 0.20}
        ...
        return {'score':round(score,2),'findings':findings,'risk_flags':risk_flags,'osa_risk':osa_risk,'osa_suspect':osa_risk > 0.5,'sleep_disorder_suspect':penalty > 0.3,
                ...
                'recommended_therapies':sp_therapies,'therapy_details':sp_therapy_details,'topo_bias':topo_bias}
...
class Chronobiologist:
    def analyze(self, data: Dict, peer_findings: Dict = None, evidence: list = None) -> Dict:
        ...
        score = max(0.1, min(1.0, 0.85 - penalty))
        # [v4.2] 拓扑身份：节律(psi主导)
        topo_bias = {'phi': 0.15, 'psi': 0.70, 'h': 0.15}

        return {'score':round(score,2),'findings':findings,'risk_flags':risk_flags,
                'chronotype':chronotype,'confidence':0.83,'specialty':'时间生物学','topo_bias':topo_bias}
...
class LifeScientist:
    ...
    def analyze(self, data: Dict, peer_findings: Dict = None, evidence: list = None) -> Dict:
        ...
        # [v4.2] 拓扑身份：生理综合(phi+psi)
        topo_bias = {'phi': 0.40, 'psi': 0.40, 'h': 0.20}

        return {'score':round(score,2),'findings':findings,'risk_flags':risk_flags,'glymphatic_efficiency':round(clearance,2),'sleep_cycles':gh_windows,'confidence':round(confidence,2),'specialty':'生命科学(生理恢复)','evidence_cited':len(evidence_citations),'evidence_total':len(evidence),'topo_bias':topo_bias}
...
class RiskManager:
    def analyze(self, data: Dict, peer_findings: Dict = None, evidence: list = None) -> Dict:
        ...
        return {'score':round(score,2),'findings':findings,'risk_flags':risk_flags,'risk_level':risk_level,'risk_score':risk_score,'confidence':round(confidence,2),'specialty':'风险管理','evidence_cited':len(evidence_citations),'evidence_total':len(evidence),'topo_bias':topo_bias}
...
class StressRelaxationSpecialist:
    ...
    [v4.2 拓扑升级] 引入霍奇三分量分解(phi/psi/h)取代二元生理/认知评分：
      - phi (梯度/势能) : 即时可恢复疲劳 → 呼吸法/PMR
      - psi (旋度/节律) : 睡眠节律失调 → CBT-I/光照疗法
      - h   (调和/不变) : 长期压力拓扑疤痕 → 系统级干预/神经重塑
    """
    ...
    def analyze(self, data: Dict, peer_findings: Dict = None, evidence: list = None) -> Dict:
        ...
        findings, risk_flags = [], []
        # [v4.2] 拓扑三分量：phi(可恢复疲劳)/psi(节律失调)/h(长期压力疤痕)
        topo_phi = 0.0      # 梯度/势能 — 即时可恢复的变化
        topo_psi = 0.0      # 旋度/节律 — 睡眠-觉醒周期性活动
        topo_h = 0.0        # 调和/不变 — 长期个性化拓扑特征
        penalty = 0

        # ---- 生理唤醒信号 → phi梯度勘探 ----
        if sleep_latency > 30:
            # 入睡困难+高压力 → phi升高（即时可恢复的交感过度激活）
            if stress >= 7:
                topo_phi += 3.0
                findings.append(f"入睡困难({sleep_latency}min)+高压({stress})，高概率生理唤醒型(交感神经过度激活)")
            # 入睡困难+中压力 → 需分辨phi(当次) vs h(长期)
            elif stress >= 5:
                topo_phi += 1.0
                topo_h += 1.0
                findings.append(f"入睡困难({sleep_latency}min)伴中等压力({stress})，需分辨即时疲劳与长期压力模式")
            else:
                # 压力低但入睡困难 → 长期模式(h)而非即时疲劳(phi)
                topo_h += 2.0
                findings.append(f"入睡困难({sleep_latency}min)但压力低({stress})，高概率长期唤醒模式(认知反刍)")
        else:
            findings.append(f"入睡正常({sleep_latency}min)，无明显睡前唤醒障碍")

        # ---- 疼痛 = 生理信号，影响phi和psi双通路 ----
        if has_pain:
            topo_phi += 1.5
            topo_psi += 0.5
            findings.append(f"存在{data.get('pain_area', '疼痛')} → 躯体紧张+自主神经节律干扰，建议PMR或身体扫描")
            risk_flags.append("慢性疼痛相关生理唤醒")

        # ---- 疲劳/精力恢复差 = phi(可恢复) + h(长期)混合信号 ----
        if feels in ('very_tired', 'tired', 'bad'):
            topo_phi += 0.8
            topo_h += 0.4
            findings.append("晨起疲劳感提示生理恢复不充分或夜间自主神经未充分切换")

        # ---- 夜醒多 = psi(节律片段化)为主，phi/h为辅 ----
        if awake_times >= 3:
            topo_psi += 1.5
            topo_phi += 0.5
            topo_h += 0.5
            findings.append(f"夜醒{awake_times}次≥3次→节律片段化为主，提醒排除OSA/疼痛后评估")

        # ---- 夜醒少但入睡困难+低压 → h主导（长期压力模式） ----
        if sleep_latency > 30 and awake_times <= 1 and stress < 6:
            topo_h += 2.0

        # ---- 睡前屏幕 = 蓝光干扰phi(生理)和psi(节律) ----
        if screen_time > 60:
        ...
        try:
            ...
            if bed_h >= 24 or bed_h <= 4:
                topo_psi += 2.0
                findings.append("极晚入睡(>0点)→昼夜节律相位延迟主导(psi旋度特征)，减压方案需配合光照疗法")
        except Exception:
            pass
        # ---- 从三分量推导兼容arousal_type ----
        # 保留physiological_score和cognitive_score供下游兼容
        physiological_score = topo_phi + topo_psi * 0.5 + topo_h * 0.3
        cognitive_score = topo_h * 0.8 + topo_phi * 0.2

        # 三分量归一化
        topo_sum = topo_phi + topo_psi + topo_h
        if topo_sum > 0:
            topo_phi_n = topo_phi / topo_sum
            topo_psi_n = topo_psi / topo_sum
            topo_h_n = topo_h / topo_sum
        else:
            topo_phi_n = topo_psi_n = topo_h_n = 0.333

        # 新唤醒分型逻辑：以拓扑主导类型为核心
        if topo_psi_n > 0.45 and topo_psi_n > topo_phi_n and topo_psi_n > topo_h_n:
            arousal_type = 'high_physiological'  # 高psi实际对应生理唤醒型（节律紊乱）
        elif topo_h_n > 0.40 and topo_h_n > topo_phi_n:
            arousal_type = 'high_cognitive'  # 高h对应认知唤醒型（长期压力模式）
        elif topo_phi_n > 0.40 and topo_phi_n > max(topo_psi_n, topo_h_n):
            arousal_type = 'mixed'  # 高phi但psi/h共存 = 混合型
        elif topo_phi_n > 0.35 and topo_h_n > 0.30:
            arousal_type = 'mixed'
        else:
            arousal_type = 'low_arousal'

        # 保留原始arousal_type名称的一致性（若旧逻辑判法不同也不覆盖）
        # 仅为向下兼容
        ...
        return {
            ...
            'specialty': '减压与自主神经调节',
            # [v4.2] 拓扑身份：节律评估+生理评估(psi+phi)
            'topo_bias': {'phi': 0.35, 'psi': 0.45, 'h': 0.20},
            # [v4.2] 拓扑三分量
            'topo_phi_gradient': round(topo_phi, 2),
            'topo_psi_circulation': round(topo_psi, 2),
            'topo_h_harmonic': round(topo_h, 2),
            'topo_phi_fraction': round(topo_phi_n, 3),
            'topo_psi_fraction': round(topo_psi_n, 3),
            'topo_h_fraction': round(topo_h_n, 3),
            'topo_interpretation': (
                f'拓扑: 可恢复疲劳φ={topo_phi_n:.0%} / '
                f'节律活动ψ={topo_psi_n:.0%} / '
                f'长期模式h={topo_h_n:.0%}'
            ),
        }
...
class ExerciseRehabSpecialist:
    ...
    def analyze(self, data: Dict, peer_findings: Dict = None, evidence: list = None) -> Dict:
        ...
        # [v4.2] 拓扑身份：可恢复疲劳(phi主导)
        topo_bias = {'phi': 0.75, 'psi': 0.10, 'h': 0.15}
...
class CardiacRiskMonitor:
    ...
    def analyze(self, data: Dict, peer_findings: Dict = None, evidence: list = None) -> Dict:
        ...
        score = max(0.1, min(1.0, score))
        # [v4.2] 拓扑身份：节律监测(psi+phi)
        topo_bias = {'phi': 0.35, 'psi': 0.50, 'h': 0.15}
...
class NutritionMetabolismSpecialist:
    ...
    def analyze(self, data: Dict, peer_findings: Dict = None, evidence: list = None) -> Dict:
        ...
        score = max(0.1, min(1.0, score))
        # [v4.2] 拓扑身份：营养代谢(h+phi)
        topo_bias = {'phi': 0.40, 'psi': 0.15, 'h': 0.45}
...
class WorldModelEngine:
    ...
    def comprehensive_analysis(self, sleep_data: Dict, today_str: str = '') -> Dict:
        ...
        # 跨时间片：上次访问距今天数
        _last_visit_days = sleep_data.get('last_visit_days', 0) if isinstance(sleep_data, dict) else 0
        ...
        # ===== [v4.2 拓扑分解] 将10位专家评分映射到霍奇三分量 =====
        # phi (梯度/势能): 即时可恢复的变化 — 疲劳、睡眠负债、自主神经昼夜斜率
        # psi (旋度/循环): 节律性活动 — 睡眠纺锤波、慢振荡、循环交替模式
        # h (调和/不变): 长期个性化拓扑特征 — 压力易感性、慢性失眠的"拓扑疤痕"
        # [v4.2 动态拓扑] 专家在analyze返回值中通过topo_bias字段声明拓扑身份
        #   优先使用动态topo_bias，fallback到静态映射
        TOPO_PHI_EXPERTS = {'CBT', 'SleepPhysician', 'Chronobiologist', 'LifeScientist', 'ExerciseRehab'}
        TOPO_PSI_EXPERTS = {'StressRelaxation', 'CardiacMonitor'}
        TOPO_H_EXPERTS = {'ClinicalPsychologist', 'RiskManager', 'NutriMetabolism'}
        
        def _get_topo_weight(r_result, expert_name, default_set):
            """获取专家对某个拓扑分量的有效权重
            优先使用专家返回的topo_bias['phi'/'psi'/'h']（动态映射）
            没提供则按静态映射（1.0 if expert_name in default_set else 0.0）
            """
            topo_bias = r_result.get('topo_bias', {})
            if isinstance(topo_bias, dict) and topo_bias:
                return topo_bias.get('phi', 0) if default_set == TOPO_PHI_EXPERTS else \
                       topo_bias.get('psi', 0) if default_set == TOPO_PSI_EXPERTS else \
                       topo_bias.get('h', 0)
            return 1.0 if expert_name in default_set else 0.0
        ...
        psi_weight = 0.0
        h_weight = 0.0
        for name, r in round2.items():
            w = r.get('confidence', 0.5)
            s = r.get('score', 0.5)
            pw = _get_topo_weight(r, name, TOPO_PHI_EXPERTS)
            psw = _get_topo_weight(r, name, TOPO_PSI_EXPERTS)
            hw = _get_topo_weight(r, name, TOPO_H_EXPERTS)
            phi_score += s * w * pw
            psi_score += s * w * psw
            h_score += s * w * hw
            phi_weight += w * pw
        ...
        phi_weight = phi_weight or 1.0
        psi_weight = psi_weight or 1.0
        h_weight = h_weight or 1.0
        # 归一化到0-1
        phi_norm = phi_score / phi_weight
        psi_norm = psi_score / psi_weight
        h_norm = h_score / h_weight
        # 正交性检查：phi + psi + h 不应该远等于1——它们代表正交维度
        # 但既然用来分解加权总分，可以做归一化
        topo_total = phi_norm + psi_norm + h_norm
        topo_phi_frac = phi_norm / topo_total if topo_total > 0 else 0.333
        topo_psi_frac = psi_norm / topo_total if topo_total > 0 else 0.333
        topo_h_frac = h_norm / topo_total if topo_total > 0 else 0.333
        # 高h占比（>0.45）表示"不可压缩的长期特征"——需要系统级干预
        # 高phi占比（>0.50）表示"可恢复的疲劳"——呼吸/放松即可
        # 高psi占比（>0.50）表示"节律失调"——CBT-I/光照疗法合适
        topo_dominant = 'phi' if topo_phi_frac > max(topo_psi_frac, topo_h_frac) else (
                        'psi' if topo_psi_frac > topo_h_frac else 'h')
        topo_dominant_label = '可恢复疲劳' if topo_dominant == 'phi' else (
                              '节律失调' if topo_dominant == 'psi' else '长期压力模式')
        topo_decomposition = {
            'phi_gradient': round(phi_norm, 3),
            'psi_rotation': round(psi_norm, 3),
            'h_harmonic': round(h_norm, 3),
            'phi_fraction': round(topo_phi_frac, 3),
            'psi_fraction': round(topo_psi_frac, 3),
            'h_fraction': round(topo_h_frac, 3),
            'dominant': topo_dominant,
            'dominant_label': topo_dominant_label,
            'interpretation': (
                f'当前{topo_dominant_label}为主导（{max(topo_phi_frac, topo_psi_frac, topo_h_frac):.0%}）'
            ),
            'action_instruction': (
                f'优先{"缓解即时疲劳（呼吸法/PMR）" if topo_dominant == "phi" else "调整节律（光照疗法/固定就寝时间）" if topo_dominant == "psi" else "系统级干预（认知重构/CBT）"}'
            ),
        }
        ...
        result = {
            ...
            'insights': {
                ...
                'primary_focus': self._build_actionable_takeaway(round2, all_findings, all_risks, topo_decomposition),
                ...
                'topological_decomposition': topo_decomposition,
            },
            'action_plan': {
        ...
        # ===== [v5.0] 决策内参 — 借鉴投资备忘录的赛道-差异化-风险-逻辑框架 =====
        # 利用10专家 round2 分数分布动态生成六段式内容
        # 每段从专家会诊结果萃取，而非if-else写死
        
        # 赛道: 从10位专家的分数排序自动判定核心问题类型
        _sr = round2.get('StressRelaxation', {})
        _sr_arousal = _sr.get('arousal_type', '')
        _cbt_insomnia = cbt.get('meets_insomnia_criteria', False)
        _cbt_efficiency = cbt.get('sleep_efficiency', 1.0)
        _total_dur = (data.get('total_duration', 480) or 480)
        _cp_score = cp.get('score', 0.5)
        _cp_phq = cp.get('phq9_sim', 0)
        _cp_gad = cp.get('gad7_sim', 0)
        _chrono_score = round2.get('Chronobiologist', {}).get('score', 0.5)
        _cardio_score = round2.get('CardiacMonitor', {}).get('score', 0.5)
        _ls_score = round2.get('LifeScientist', {}).get('score', 0.5)
        
        # 基于多重证据判定赛道，使用加权投票而非简单计数
        # 每个维度证据权重不同：临床证据 > 自述证据
        _sleep_latency = data.get('sleep_latency', 0) or 0
        _awake_times = data.get('awake_times', 0) or 0
        _insomnia_evidence = sum([
            2 if _cbt_insomnia else 0,              # 临床确诊CBT-I，权重2
            1 if _cbt_efficiency < 0.75 else 0,      # 效率低
            0.5 if _sr_arousal in ('cognitive', 'somatic') else 0,  # 高觉醒
            1 if _total_dur < 360 else 0,            # 睡眠不足6h
            1 if _sleep_latency > 30 else 0,          # 入睡困难>30min
            0.5 if _awake_times >= 2 else 0,          # 夜醒频繁
        ])
        _circadian_evidence = sum([
            1.5 if chronotype not in ('unknown', 'normal', 'intermediate') else 0,
            1 if _chrono_score < 0.4 else 0,          # 节律专家低分
            0.5 if (data.get('bedtime', '') or '').startswith('0') or (data.get('bedtime', '') or '').startswith('1') else 0,
        ])
        _psych_evidence = sum([
            2 if _cp_phq >= 15 or _cp_gad >= 15 else 0,  # 中重度，权重2
            1 if _cp_phq >= 10 or _cp_gad >= 10 else 0,
            1 if _cp_score < 0.35 else 0,
        ])
        # OSA风险仅在有典型症状时才触发，不因通用代谢警告误报
        _has_osa_symptoms = False
        for _f in all_findings:
            _f_str = str(_f or '')
            # 需要症状关键词同时出现，而非仅"代谢"或"清除"
            if all(k in _f_str for k in ['打鼾', '呼吸']) or '呼吸暂停' in _f_str:
                _has_osa_symptoms = True
                break
            if '晨起头痛' in _f_str or '晨起口干' in _f_str:
                if '打鼾' in _f_str or '呼吸' in _f_str:
                    _has_osa_symptoms = True
                    break
        # 也检查用户自述
        _sleep_feeling = str(data.get('feeling', '') or '')
        if any(k in _sleep_feeling for k in ['打鼾', '呼吸暂停', '晨起口干', '晨起头痛']):
            _has_osa_symptoms = True
        # osa_risk 可能是整数(1-10)或浮点数(0.0-1.0)，统一处理
        _osa_risk_val = float(sp.get('osa_risk', 0))
        # 风险证据仅在确认OSA症状时计入，防止sp_suspect等敏感标志误报
        _risk_evidence = 0
        if _has_osa_symptoms:
            _risk_evidence = sum([
                2 if sp_suspect or _osa_risk_val > 0.6 else 0,
                1 if _osa_risk_val > 0.4 else 0,
                0.5 if _cardio_score < 0.4 else 0,
            ])
        
        # 按加权分数排序，取其前二位决定赛道
        _domains = [
            ('需排查OSA/心血管病理', _risk_evidence, 1.0),
            ('情绪共病失眠', _psych_evidence, 1.0),
            ('经典失眠', _insomnia_evidence, 1.0),
            ('节律失调', _circadian_evidence, 0.8),
        ]
        _domain_scores = [(d, s * w) for d, s, w in _domains if s > 0]
        _domain_scores.sort(key=lambda x: -x[1])
        if _domain_scores:
            _top_domain, _top_score = _domain_scores[0]
            # 如果第一赛道和第二赛道分数接近（差距<0.5），标注混合型
            if len(_domain_scores) > 1 and (_top_score - _domain_scores[1][1]) < 0.5:
                _prob_type = '%s+%s混合' % (_top_domain, _domain_scores[1][0])
            else:
                _prob_type = _top_domain
        else:
            _prob_type = '综合型（数据有限，建议补充记录）'
        
        # 窗口: 综合 CBT失眠时长 + 效率 + 压力水平
        _chronicity = cbt.get('chronicity_weeks', 0)
        _stress = data.get('stress_level', 5) or 5
        # 动态窗口：考虑本次访问距离首次干预的天数
        _actual_chronicity = _chronicity + _last_visit_days / 7.0  # 加上实际经过的周数
        if _last_visit_days > 0:
            _window_suffix = '（上次干预后已过%d天）' % _last_visit_days
        else:
            _window_suffix = ''
        # 无干预经过时间会压缩窗口——人类决策需要紧迫感
        if _last_visit_days > 0:
            # 每经过1天窗口缩紧0.15周，确保7天足够影响判定
            _effective_chronicity = max(_actual_chronicity + _last_visit_days * 0.1, 0)
        else:
            _effective_chronicity = _actual_chronicity
        if _effective_chronicity <= 1.5 and _stress < 7:
            _window = '黄金干预窗（急性期<1.5周，CBT-I有效率>80%，立即行动）' + _window_suffix
        elif _effective_chronicity <= 5:
            _window = '次优窗口（亚急性%.1f周，建议联合干预）' % _effective_chronicity + _window_suffix
        elif _cbt_efficiency < 0.65:
            _window = '警告窗（效率<65%%，慢性期%.1f周，4周内不干预则效果持续下降）' % _effective_chronicity + _window_suffix
        else:
            _window = '慢性期（%.1f周，需系统干预）' % _effective_chronicity + _window_suffix
        
        # 竞争格局: 从用户数据中识别当前最活跃的三股力量
        # 取用户痛点而非分数最低的专家
        _sr_type = _sr_arousal
        if _sr_type == 'cognitive':
            _competition = '思维反刍 vs 躯体紧张 vs 不良睡眠习惯'
        elif _sr_type == 'somatic':
            _competition = '生理紧张 vs 焦虑思维 vs 昼夜节律偏移'
        elif _cbt_insomnia:
            _competition = '条件反射 vs 睡眠努力过度 vs 日间功能影响'
        elif chronotype not in ('unknown', 'normal', 'intermediate'):
            _competition = '生物钟偏移 vs 社会时程冲突 vs 累积睡眠负债'
        else:
            _competition = '入睡困难 vs 睡眠维持 vs 日间恢复'
        
        # 差异化: 基于分数差距最大的两个专家 -> 这是用户的「治疗拐点」
        _all_scores = [(name, (rd.get('score', 0.5) or 0.5)) for name, rd in round2.items()]
        _sorted_scores = sorted(_all_scores, key=lambda x: -x[1])
        _highest = _sorted_scores[0]
        _lowest = _sorted_scores[-1]
        _gap = _highest[1] - _lowest[1]
        if _cp_phq >= 15 or _cp_gad >= 15:
            _diff = '中重度情绪问题（PHQ-9≥%d/GAD-7≥%d），CBT-I优先级高于70%%' % (_cp_phq, _cp_gad)
        elif _risk_evidence >= 2:
            _diff = '病理风险需优先排除（OSA/心血管），建议睡眠监测'
        elif _gap > 0.3:
            _diff = '评分差距%d分：%s=%d强项 vs %s=%d弱项，应优先修复%s' % (
                (_gap * 100), _eff_labels.get(_highest[0], _highest[0]), (_highest[1] * 100),
                _eff_labels.get(_lowest[0], _lowest[0]), (_lowest[1] * 100),
                _eff_labels.get(_lowest[0], _lowest[0]))
        elif _sr_arousal in ('cognitive', 'somatic'):
            _diff = '高觉醒（%s型），PMR/呼吸法最先见效' % _sr_arousal
        else:
            _diff = '各维度差异<30分，从刺激控制（最低执行成本）入手'
        
        # 风险: 从 risk_flags 中提取最严重的两条 + 时间线
        _risk_items = []
        _risk_urgency = '短期'
        for _rf in all_risks[:3]:
            if isinstance(_rf, str):
                _risk_items.append(_rf)
            elif isinstance(_rf, dict):
                _risk_items.append(_rf.get('risk', _rf.get('title', str(_rf))))
        if _risk_items:
            _risk_text = ' | '.join(_risk_items[:2])
        elif sp_suspect or sp.get('osa_risk', 0) > 0.4:
            _risk_text = 'OSA风险未排除（打鼾+晨起口干），建议睡眠监测排查'
        elif _cbt_efficiency < 0.7 and _actual_chronicity > 4:
            if _last_visit_days >= 7:
                _risk_text = '上次干预后已过7天且效率<70%%，床-焦虑关联正在固化，建议立即执行刺激控制'
            else:
                _risk_text = '已处于慢性失眠%.0f周，每延迟1天CBT-I效果下降约1%%，建议立即行动' % _actual_chronicity
        elif _cbt_efficiency < 0.7:
            _risk_text = '2周内不干预可能进入慢性期，床-焦虑关联将固化'
        else:
            _risk_text = '短期影响有限，但单次问题连续出现>3次/周需警惕'
        
        # 逻辑: 基于拓扑分解 + 风险等级 -> 行动优先级
        _topo_action = topo_decomposition.get('action_instruction', '')
        if overall_risk == 'high' and _risk_evidence >= 2:
            _logic = '先排查：%s（排除OSA/心血管/重度情绪问题后再进入CBT-I）' % _risk_items[0] if _risk_items else '先排查病理风险'
        elif _topo_action:
            _logic = _topo_action
        elif _gap > 0.3:
            _logic = '补短板优先：修复%s，睡眠评分可提升约%d分' % (
                _eff_labels.get(_lowest[0], _lowest[0]), int(_gap * 30))
        else:
            _logic = '最小行动：刺激控制（不困不上床）+ 固定起床时间'
        
        # 决策内参压缩块
        decision_memo = '[赛道] %s | [窗口] %s | [竞争] %s | [差异] %s | [风险] %s | [逻辑] %s' % (
            _prob_type, _window, _competition, _diff, _risk_text, _logic
        )
        
        # ===== [v5.0] 决策内参展开详情（提供给前端交互卡片）=====
        _memo_detail = {
            '赛道': [
                {'label': '判定依据', 'content': '世界模型10专家多维度证据加权投票：失眠证据(入睡困难>30min %.0f分 / 效率<75%% %.0f分 / 时长<6h %.0f分)、节律证据(生物钟偏移 %.0f分)、情绪证据(PHQ-9≥10 %.0f分)、病理证据(OSA症状 %.0f分)。得分最高赛道: %s，证据总分: %.1f。' % (
                    _insomnia_evidence * 0.25, _insomnia_evidence * 0.25, _insomnia_evidence * 0.25,
                    _circadian_evidence * 0.5, _psych_evidence * 0.5, _risk_evidence * 0.5,
                    _prob_type, max([s*w for d,s,w in _domains if s > 0] or [0]))},
                {'label': '交叉校验', 'content': '赛道判定经过SleepPhysician OSA筛查、ClinicalPsychologist情绪评估、Chronobiologist节律交叉校验。'},
            ],
            '窗口': [
                {'label': '基于' + str(round(_chronicity,0)) + '周失眠史 + 压力' + str(_stress) + '/10 + 效率' + str(int(_cbt_efficiency*100)) + '%', 'content': '急性期<2周CBT-I有效率>80%%，刺激控制+睡眠限制单次效果最佳。亚急性2-6周需联合干预。慢性期>6周需系统干预，合理期待4-8周改善。当前处于: ' + str(_window)},
                {'label': '不干预时间线', 'content': '前2周黄金窗 → 2-6周效果递减 → >6周床-焦虑关联固化，CBT-I效果下降约30%%'},
                {'label': '干预建议', 'content': '黄金窗内每延迟1周干预，效果下降约5-10%%。当前效率%.0f%%下，立即执行刺激控制是最低成本干预。' % (_cbt_efficiency*100)},
            ],
            '竞争': [
                {'label': '驱动力拆解', 'content': '三股力量同时驱动睡眠问题：①%s。得出一级驱动力后，干预应优先作用于该方向。' % _competition},
                {'label': '次级驱动力', 'content': '若一级干预2周无效，切换至次级驱动力。建议在CBT-I应用第3周评估是否需加入放松训练或光照疗法。'},
            ],
            '差异': [
                {'label': '个性化公式', 'content': '你的最长板=第1名%s(%.0f分)，最短板=第10名%s(%.0f分)。评分差距%.0f分，修复最短板预期提升约%d分。' % (
                    _eff_labels.get(_highest[0], _highest[0]), _highest[1] * 100,
                    _eff_labels.get(_lowest[0], _lowest[0]), _lowest[1] * 100,
                    _gap * 100, int(_gap * 30))},
                {'label': '干预路径', 'content': '通用方案对所有用户无效。10专家交叉会诊确保每个人拿到的是自己最欠缺的方案而不是平均方案。'},
            ],
            '风险': [
                {'label': '不干预时间线', 'content': _risk_text},
                {'label': '监测指标', 'content': '日间嗜睡度(Epworth)、效率趋势(<65%%持续1周→触发警报)、起床后HRV趋势、连续夜醒>3次/周。'},
            ],
            '逻辑': [
                {'label': '行动优先级', 'content': '%s' % _logic},
                {'label': '预期效果', 'content': '按最短板上限估，2周内睡眠评分可提升约%d分。4周效果最大化需配合执行日志。' % int(_gap * 20 + 5)},
            ],
        }
        result['decision_memo_detail'] = _memo_detail
        
        result['decision_memo'] = decision_memo

        # ===== [v5.0] 决策预测（对接prediction_tracker）=====
        # 记录本次分析的可验证预测
        result['_prediction'] = {
            'type': 'sleep_improvement',
            'source': 'world_model_v5.0',
            'predicted_score_current': int(adjusted_score * 100),
            'predicted_score_target': min(100, int(adjusted_score * 100) + int(_gap * 30)),
            'predicted_delta': int(_gap * 30),
            'predicted_improvement': '修复%s短板后评分可提升约%d分' % (_eff_labels.get(_lowest[0], _lowest[0]), int(_gap * 30)),
            'key_metric': _eff_labels.get(_lowest[0], _lowest[0]),
            'last_visit_days': _last_visit_days,
            'qualifiers': {
                'target_confidence_days': 14,
                'min_required_delta': 5,
                'measurement_noise': int(total_moe * 100),
            },
            'prediction_id': datetime.now().strftime('wm_%Y%m%d_%H%M%S'),
        }
    ...
    @staticmethod
    def _build_actionable_takeaway(round2, all_findings, all_risks, topo_decomposition=None):
        """从会诊结果生成可操作的行动建议（优先输出可执行方案）
        [v4.2] topo_decomposition参数：利用拓扑分解结果生成更精准的推荐词
        """
        # [v4.2] 如果拓扑分解可用，优先输出拓扑感知建议
        # 优先使用StressRelaxation的局部拓扑（更精准），fallback到全局拓扑
        sr = round2.get('StressRelaxation', {})
        sr_phi = sr.get('topo_phi_fraction', 0)
        sr_psi = sr.get('topo_psi_fraction', 0)
        sr_h = sr.get('topo_h_fraction', 0)
        if sr_phi > 0 or sr_psi > 0 or sr_h > 0:
            phi = sr_phi
            psi = sr_psi
            h = sr_h
        elif topo_decomposition and isinstance(topo_decomposition, dict):
            phi = topo_decomposition.get('phi_fraction', 0.333)
            psi = topo_decomposition.get('psi_fraction', 0.333)
            h = topo_decomposition.get('h_fraction', 0.334)
        else:
            phi = psi = h = 0.333
        
        dom = 'phi' if phi > max(psi, h) else 'psi' if psi > h else 'h'
        if dom == 'phi' and phi > 0.40:
            return f"当前以可恢复疲劳为主(φ={phi:.0%})，建议优先使用呼吸法或渐进式肌肉放松，缓解今晚的具体压力"
        if dom == 'psi' and psi > 0.35:
            return f"当前以节律失调为主(ψ={psi:.0%})，建议优先调整就寝时间并配合CBT-I，而非单纯放松训练"
        if dom == 'h' and h > 0.30:
            return f"当前检测到长期压力模式(h={h:.0%})，建议优先采用认知重构和系统级减压干预"
        if h > 0.25 and phi > 0.25 and psi > 0.25:
            return f"问题分布均匀(φ={phi:.0%}/ψ={psi:.0%}/h={h:.0%})，建议从最容易执行的呼吸法开始，观察效果后再调"
