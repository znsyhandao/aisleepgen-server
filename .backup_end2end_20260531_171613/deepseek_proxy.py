...
__version__ = '20260522_1'  # +face-analyze enrichment
...
class ProxyHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if SECURITY_BRIDGE_AVAILABLE and path not in ('/health',):
            ...
            if not allow:
                self._set_headers(json=resp if resp else {'error': 'forbidden'})
                self.wfile.write(_send_json(resp if resp else {'error': 'forbidden', 'status': 403}).encode('utf-8'))
                return
    ...
    def do_POST(self):
        ...
        if SECURITY_BRIDGE_AVAILABLE:
            ...
            if not allow:
                self._set_headers(json=resp_override if resp_override else {'error': reason})
                self.wfile.write(_send_json(resp_override if resp_override else {'error': reason, 'status': 403}).encode('utf-8'))
                return
    ...
    def _handle_delete_user_data(self, data):
        ...
        self.wfile.write(json.dumps({
            ...
            'deleted_analysis': deleted_count if 'deleted_count' in dir() else 0,
...
"""
        ...
        try:
            ...
            _has_ask = any(p in reply_content for p in _ask_patterns)
            if _has_ask:
                # 从缓存拉实时数据拼回复
                _pp_profile = _load_user_profile(openid)
                _pp_sd = _pp_profile.get('latest', {}).get('sleep_data', {}) or {}
                _pp_bt = _pp_sd.get('bedtime', '?')
                _pp_wt = _pp_sd.get('wake_time', '?')
                _pp_sc = _pp_profile.get('latest', {}).get('score', '?')
                _pp_mi = _pp_profile.get('user_info', {}).get('main_issue', '')
                _pp_parts = [f'入睡{_pp_bt}', f'起床{_pp_wt}', f'评分{_pp_sc}']
                if _pp_mi:
                    _pp_parts.append(f'主诉{_pp_mi}')
                _pp_context = '，'.join(_pp_parts)
                reply_content = f'能理解你的困扰。根据记录：{_pp_context}。直接基于此分析：压力偏高可能是深睡不足主要原因，建议优先关注睡前放松。你今晚和之前比有变化吗？'
                # 写 flag 确认
                try:
                    import os as _os9
                    _p9 = _os9.path.join(_os9.path.dirname(_os9.path.abspath(__file__)), '_postproc_flag.txt')
                    with open(_p9, 'w', encoding='utf-8') as _f9:
                        _f9.write(f'intercepted at {time.time()}\n')
                except Exception as _ee: print(f"[except] L{0}: {_ee}".format(4483))
            ...
            try:
                _ad_tids = None
                if 'wm_result_global' in dir() and wm_result_global:
            ...
            try:
                from medical_filter import filter_response
                ...
                _mr_filtered, _mr_risk = filter_response(_mr_openid, _mr_user_msg, response_obj.get('reply', ''))
