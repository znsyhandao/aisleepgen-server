...
class MetaLearner:
    ...
    def _is_oscillating(self, param_name: str, new_value: float) -> bool:
        """检测参数是否在短时间内反复震荡（双适配器抑制机制）
        如果同一个参数在最近3次review中被反复调高又调低，说明旧知识可能过时
        """
        try:
            log = self._load_meta_log()
            recent = []
            for r in reversed(log.get('adjustments', [])[-15:]):  # 最近15条调整记录
                if r.get('param') == param_name:
                    recent.append(r.get('new_value', 0))
            if len(recent) >= 3:
                # 检测震荡：连续3次以上方向交替变化
                directions = []
                for i in range(1, min(len(recent), 6)):
                    directions.append(recent[i] - recent[i-1])
    ...
    def apply_adjustments(self, review_result, openid='_system'):
        ...
        # ═══ 双适配器：写/抑制 路由器（来自Rout-Specialized Dual Adapter论文启发）═══
        # 写适配器：决定哪些新知识需要写入
        # 抑制适配器：决定哪些旧知识已过时，标记为不活跃
        write_adapter = []  # 要写入的新调整
        suppress_adapter = []  # 要标记为过时的旧参数
        for adj in adjustments:
            # 用参数变化幅度判断：大变化→可能是新知识（写），小变化→可能是噪声（抑制旧知识）
            param_name = adj['param']
            new_value = adj['new']
            if param_name in self.param_history._cache:
                old_value = self.param_history._cache.get(param_name, {}).get('value', 0)
                delta = abs(new_value - old_value)
                # 如果变化幅度 > 30% 旧值，认为是新知识（写模式）
                if old_value != 0 and delta / (abs(old_value) + 1e-8) > 0.3:
                    write_adapter.append(adj)
                    _ml_log.info('[DualAdapter] WRITE: %s %.4f→%.4f (Δ%.1f%%)', param_name, old_value, new_value, delta/abs(old_value)*100)
                # 如果参数在短期内（< 3次review）被反复调整（震荡），可能是过时知识→抑制
                elif self._is_oscillating(param_name, new_value):
                    suppress_adapter.append(adj)
                    _ml_log.info('[DualAdapter] SUPPRESS: %s (振荡检测，标记为候选过时)', param_name)
                else:
                    write_adapter.append(adj)  # 小变化正常写入
            else:
                write_adapter.append(adj)  # 没有历史记录，直接写
        
        # 对写适配器和抑制适配器分别生成不同摘要
        if write_adapter:
            _ml_log.info('[DualAdapter] 写适配器: %d项新知识待写入', len(write_adapter))
        if suppress_adapter:
            _ml_log.info('[DualAdapter] 抑制适配器: %d项参数被标记为过时候选', len(suppress_adapter))
        
        # 安全钳制：对写适配器的参数应用
        for adj in write_adapter:
        ...
        # v1 Subspace Sharing: 将所有调整向量投影到共享子空间
        if adjustments:
            try:
                subspace = get_subspace_optimizer()
                subspace._review_count += 1
                # 把本次调整向量收集起来
                for adj in applied:
                    ov = adj.get('from', 0)
                    nv = adj.get('to', 0)
                    if ov != nv:
                        subspace.project([nv - ov], adj.get('param', '_unknown'))
                # 定期重建子空间
                if subspace._review_count >= subspace.rebuild_interval:
                    subspace.rebuild_subspace()
                    subspace._review_count = 0
            except Exception as _sse:
                _ml_log.warning('[Subspace] projection failed: %s', _sse)
...
# ===== SubspaceSharedOptimizer — Sparse Subspace-to-Expert Sharing v1 =====
# CONCEPT_PROOF: 所有用户的参数调整通过低维子空间共享学习
# 论文: Sparse Subspace-to-Expert Sharing (2026)
# 
# 作用: 替代独立全参数更新，所有用户共享子空间基 + 各用户只学少量系数
# 每次 apply_adjustments 时自动投影

class SubspaceSharedOptimizer:
    """子空间共享优化器
    
    所有用户的参数调整向量共享一个低维子空间：
    - 全局维护 SVD 子空间基 U (n_params x k)
    - 每个用户只存储 k 维系数向量 α
    - 参数更新 = U @ α (约束在子空间内)
    - 每 N 轮 review 后重算子空间
    """
    
    def __init__(self, subspace_rank=4, rebuild_interval=3):
        self.rank = subspace_rank  # 子空间维度 k
        self.rebuild_interval = rebuild_interval  # 每 N 轮重算
        self.U = None              # 子空间基
        self.user_coeffs = {}      # user -> alpha vector
        self._review_count = 0
        self._param_buffer = []    # buffer of adjustment vectors for rebuild
        self._logger = logging.getLogger('aisleepgen.subspace_optimizer')
    
    def get_subspace_count(self):
        """返回当前有效的子空间数量"""
        if self.U is None:
            return 0
        return self.U.shape[1]
    
    def project(self, param_vector, openid="_global", sparsity_lambda=0.0):
        """将参数调整向量投影到子空间
        支持 sparsity_lambda > 0 时对系数施加 L1 稀疏（TEVI 稀疏编码思路）
        
        如果子空间已存在:
          1. 计算最优系数: α = U^T @ v
          2. 复原投影: v_proj = U @ α
          3. 返回 v_proj (只在子空间内变化)
        
        如果子空间不存在:
          直接返回原向量，缓存用于后续重建
        """
        v = np.array(param_vector, dtype=np.float64)
        if v.ndim == 0:
            v = np.array([v])
        if v.ndim > 1:
            v = v.flatten()
        ...
        if self.U is not None:
            # 投影到子空间
            alpha = self.U.T @ v  # k 维系数
            if sparsity_lambda > 0:
                # L1 稀疏: sign * max(|coeff| - lambda, 0)
                alpha = np.sign(alpha) * np.maximum(np.abs(alpha) - sparsity_lambda, 0.0)
            v_proj = self.U @ alpha  # 还原为 n 维
            
            # 更新用户的系数表征
            self.user_coeffs[openid] = alpha.tolist()
            
            return v_proj.tolist()
        
        # 子空间不存在：缓冲数据，用于下次构建
        if len(param_vector) > 0:
            self._param_buffer.append(v)
        return param_vector
    
    def rebuild_subspace(self):
        """基于缓冲的用户调整向量重建子空间
        
        用 SVD 分解提取前 k 个主成分作为子空间基
        """
        if len(self._param_buffer) < self.rank:
            return False
        
        X = np.stack(self._param_buffer)  # (n_samples x n_params)
        ...
        try:
            ...
            self.U = Vt[:k, :].T  # (n_params x k)
            
            self._logger.info(
                "[Subspace] Rebuilt with rank=%d, explained_var=%.3f",
                k, np.sum(S[:k]**2) / np.sum(S**2)
            )
            return True
        except Exception as e:
            self._logger.warning("[Subspace] SVD failed: %s", e)
            return False
    
    def get_user_coeff(self, openid):
        """获取用户在子空间中的系数"""
        return self.user_coeffs.get(openid, None)
    
    def get_subspace_diversity(self):
        """子空间多样性指标——系数分布熵"""
        if not self.user_coeffs:
            return 0.0
        coeffs = np.array(list(self.user_coeffs.values()))
        var = np.var(coeffs, axis=0)
        return float(np.mean(var))


_SUBSPACE_OPTIMIZER = None


def get_subspace_optimizer():
    global _SUBSPACE_OPTIMIZER
    if _SUBSPACE_OPTIMIZER is None:
        _SUBSPACE_OPTIMIZER = SubspaceSharedOptimizer()
    return _SUBSPACE_OPTIMIZER


# ===== ImplicitSampleSynthesizer — 隐式数据合成（正样本增强）=====
class ImplicitSampleSynthesizer:
    """为冷启动用户合成正样本，基于已学用户的子空间系数插值或加噪"""
    def __init__(self, optimizer, n_synthetic=3):
        self.optimizer = optimizer
        self.n_synthetic = n_synthetic

    def synthesize(self, user_coeff=None, user_params=None):
        import numpy as np
        import random as _rnd
        basis = self.optimizer.U if hasattr(self.optimizer, 'U') else getattr(self.optimizer, '_basis', None)
        if basis is None:
            return []
        if user_coeff is None:
            if user_params is None:
                return []
            user_coeff = basis.T @ np.array(user_params, dtype=np.float64)
        coeff_arr = np.array(user_coeff)
        all_coeffs = self._collect_existing_coeffs()
        if len(all_coeffs) < 2:
            return self._noise_based_synthesis(coeff_arr)
        return self._interpolation_synthesis(coeff_arr, all_coeffs)

    def _collect_existing_coeffs(self):
        import numpy as np
        coeffs = []
        cache = getattr(self.optimizer, 'user_coeffs', {})
        for uid, c in cache.items():
            if isinstance(c, (list, np.ndarray)):
                coeffs.append(np.array(c))
        return coeffs

    def _interpolation_synthesis(self, user_coeff, all_coeffs):
        import numpy as np
        import random
        synthetics = []
        n = min(self.n_synthetic, len(all_coeffs))
        selected = random.sample(all_coeffs, n)
        for other in selected:
            alpha = random.uniform(0.5, 0.9)
            mixed = alpha * user_coeff + (1 - alpha) * other
            synthetics.append(mixed.tolist())
        return synthetics

    def _noise_based_synthesis(self, user_coeff):
        import numpy as np
        return [(user_coeff + np.random.randn(len(user_coeff)) * 0.05).tolist()
