...
__version__ = '20260601_1'  # +face-analyze enrichment +dual-engine markers

# ============================================================
# 架构预留：将来拆分为
#   core_engine.py   - 世界模型 + 专家调度（当前为内联）
#   local_engine.py  - 端侧轻量推理（预留接口）
#   cloud_engine.py  - 云端深度分析（ProxyHandler）
#   profile.py       - 用户配置管理
#   butler.py        - 管家/简报
# ============================================================
...
class ProxyHandler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        # === 全链路审计 (L1+L3): 代理 wfile.write 拦截所有响应 ===
        _orig_audit_write = self.wfile.write
        _audit_path = path
        _audit_req = data if isinstance(data, dict) else {}
        _audit_ts = __import__('time').time()
        def _audit_write(data):
            if isinstance(data, bytes):
                try:
                    resp = __import__('json').loads(data.decode('utf-8'))
                    dur = int((__import__('time').time() - _audit_ts) * 1000)
                    from audit_logger import log_api_call, validate_response
                    from compliance import mask_openid, filter_sensitive
                    oid = resp.get('openid') or _audit_req.get('openid', '') or 'anonymous'
                    if oid == 'anonymous' and 'meta' in resp:
                        oid = resp['meta'].get('user_id', 'anonymous')
                    sid = resp.get('session_id') or _audit_req.get('session_id', '')
                    # 脱敏 openid 用于日志记录
                    masked_oid = mask_openid(oid) if oid != 'anonymous' else oid
                    # 过滤敏感字段
                    clean_req = filter_sensitive(_audit_req)
                    issues = validate_response(_audit_path, resp)
                    # SAE 特征子空间注入（概念验证）：在审计链中记录特征激活模式
                    from compliance import inject_sae_features
                    resp = inject_sae_features(resp, _audit_req)
                    # 在 SAE 注入后重新脱敏和编码
                    clean_resp = filter_sensitive(resp)
                    data = __import__('json').dumps(resp, ensure_ascii=False).encode('utf-8')
                    if issues:
                        resp.setdefault('_audit_warnings', []).extend(issues)
                        data = __import__('json').dumps(resp, ensure_ascii=False).encode('utf-8')
                        clean_resp = filter_sensitive(resp)
                    # 日志存储使用脱敏后 openid
                    log_api_call(masked_oid, sid, _audit_path, 'POST', clean_req, clean_resp, dur, oid)
                except Exception:
                    print(f'[except] L2189: except:')
                    pass
            _orig_audit_write(data)
        self.wfile.write = _audit_write
        ...
        elif path == '/api/companion/start':
            """POST /api/companion/start — 启动AI睡眠陪伴"""
            self._handle_companion_start(data)
        elif path == '/api/companion/update':
            """POST /api/companion/update — 更新陪伴状态"""
            self._handle_companion_update(data)
        elif path == '/api/companion/status':
            """POST /api/companion/status — 查询陪伴状态"""
            self._handle_companion_status(data)
        elif path == '/api/companion/stop':
            """POST /api/companion/stop — 停止陪伴"""
            self._handle_companion_stop(data)
    ...
    def _call_deepseek(self, messages, max_tokens=2000, temperature=0.7):
        ...
        try:
            ...
            # [乙醛酸启示③] API失败→走Plan B降级推理
            _err_msg = f'HTTP {e.code}: {e.read().decode("utf-8")}'
            try:
                _last_msg = self._build_fallback_response(messages[-1].get('content', '') if messages else '')
                if _last_msg:
                    print('[Fallback] Plan B: API HTTP error, using local fallback')
                    return {'content': _last_msg, 'usage': {}, 'fallback': True}
            except:
                pass
            ...
            try:
                _last_msg = self._build_fallback_response(messages[-1].get('content', '') if messages else '')
                if _last_msg:
                    print('[Fallback] Plan B: API exception, using local fallback')
                    return {'content': _last_msg, 'usage': {}, 'fallback': True}
            except:
                pass
            return {'error': str(e)}
    ...
    def _handle_relax_feedback(self, data):
        ...
        openid = self._get_openid(data)
        if not openid:
            self._send_json({'success': False, 'error': '缺少用户标识'})
            return
        ...
        self._send_json({'success': True, 'recorded': True})
...
助眠故事长度控制在80-150字，不分段。'''
    ...
    def _handle_clinical_report(self, data):
        ...
        profile = _load_user_profile(openid)
        if not profile:
            self._send_json({'success': False, 'error': '用户数据不存在'})
            return
        sleep_data = profile.get('sleep_data', [])
        if len(sleep_data) < 3:
            self._send_json({'success': False, 'error': '数据不足，请先记录睡眠'})
            return
...
请推荐合适的冥想计划。"""
    ...
    def _handle_memory_recall(self, data):
        ...
        if not os.path.exists(profile_path):
            self._send_json({'success': False, 'recall_text': '', 'error': 'no_data'})
            return
        try:
            ...
            self._send_json({'success': True, 'recall_text': recall})
        except Exception as e:
            self._send_json({'success': False, 'recall_text': '', 'error': str(e)})
...
"""
                # ===== 跨 session 情景记忆注入 =====
                mem_ctx = coord_result.get('_memory_context', {})
                mem_summaries = mem_ctx.get('summaries', []) if mem_ctx else []
                if mem_summaries:
                    wm_physio_context += '\n===== 用户近期记忆摘要(跨会话回顾) =====\n'
                    for s in mem_summaries:
                        wm_physio_context += f'- {s}\n'
                    wm_physio_context += '(本摘要来自用户的历史记录，帮助提供连贯的个性化支持)\n'
            except Exception as e:
...
"""
                    ...
                    # ===== [v5.2 SenNet] 趋势预测：判断状态恶化 → 建议预防干预 =====
                    _trend_forecast = ""
                    try:
                        _profile = _load_user_profile(openid) if openid else {}
                        _device = _profile.get('devices', {}).get('huawei_band', {}).get('last_sleep_data', {})
                        _sleep_history = _profile.get('devices', {}).get('huawei_band', {}).get('sleep_history', [])
                        _hrv_trend = []
                        if isinstance(_sleep_history, list):
                            # 取最近5晚的HRV和评分
                            for _night in _sleep_history[-5:]:
                                _hrv = _night.get('hrv_avg', 0) if isinstance(_night, dict) else 0
                                if _hrv:
                                    _hrv_trend.append(_hrv)
                        if len(_hrv_trend) >= 3:
                            _hrv_change = (_hrv_trend[-1] - _hrv_trend[0]) / max(len(_hrv_trend), 1)
                            if _hrv_change < -5:
                                _trend_forecast = (
                                    "===== 趋势预测(入睡前微干预建议) =====\n"
                                    "⚠️ 您的HRV连续下降趋势，预测明天下午可能认知疲劳加剧。\n"
                                    "建议：今晚入睡前做一次4-7-8呼吸练习，可降低明天状态恶化概率。\n"
                                    "==========================\n"
                                )
                            elif _hrv_change > 5:
                                _trend_forecast = (
                                    "===== 趋势预测 =====\n"
                                    "✅ HRV连续改善趋势，预测明天状态良好。保持节奏即可。\n"
                                    "==========================\n"
                                )
                    except Exception:
                        pass
                    if _trend_forecast:
                        wm_context += _trend_forecast
                    ...
                    try:
                        _safe_update_profile(full_data if 'full_data' in locals() else {},
                                             ...
                                             user_message, openid)
                    except Exception as _ee: print(f"[except] L{0}: {_ee}".format(3868))
...
"""
        ...
        try:
            ...
            # ===== [乙醛酸启示④] 代谢瓶颈阻断：检测对抗/异常流 =====
            try:
                _adv_reply = response_obj.get('reply', '')
                _adv_flag = False
                if len(_adv_reply) > 3000:
                    _adv_reply = _adv_reply[:2000] + '\n\n（回复已安全截断）'
                    _adv_flag = True
                    print('[Adversarial] Ultra-long response truncated')
                _sentences = _adv_reply.split('。')
                for _si in range(len(_sentences) - 2):
                    if _sentences[_si] and _sentences[_si] == _sentences[_si+1] == _sentences[_si+2]:
                        _cut_pos = _adv_reply.find(_sentences[_si])
                        _adv_reply = _adv_reply[:_cut_pos] + '\n\n（检测到重复模式，已截断）'
                        _adv_flag = True
                        print('[Adversarial] Repetitive pattern truncated')
                        break
                if _adv_flag:
                    response_obj['reply'] = _adv_reply
                    response_obj['_adversarial_blocked'] = True
            except Exception:
                pass
...
{{
  ...
  "sourceName": "AI聊天生成"
    ...
    # ── 睡眠AI陪伴（companion_mode 状态机）──
    def _handle_companion_start(self, data):
        """POST /api/companion/start — 启动AI睡眠陪伴会话"""
        self._set_headers()
        openid = data.get('openid', 'default')
        ...
        try:
            from companion_mode import CompanionSession
            session = CompanionSession(protocol, message)
            steps = session.get_initial_steps()

            # 保存会话实例到类级缓存
            if not hasattr(self.__class__, '_companion_sessions'):
                self.__class__._companion_sessions = {}
            self.__class__._companion_sessions[openid] = session

            result = {
                'success': True,
                'protocol_name': steps.get('protocol_name', protocol),
                'state': steps.get('state', 'CALMING'),
                'intro': steps.get('intro', ''),
                'steps': steps.get('steps', []),
                'total_cycles': steps.get('total_cycles', 5),
                'is_scan': steps.get('is_scan', False),
                'areas': steps.get('areas', []),
                'total_areas': steps.get('total_areas', 0),
            }
            self.wfile.write(json.dumps(result, ensure_ascii=False).encode('utf-8'))
        except Exception as e:
            import traceback; traceback.print_exc()
            self._set_headers(500)
            self.wfile.write(json.dumps({'success': False, 'error': str(e)[:80]}).encode('utf-8'))

    def _handle_companion_update(self, data):
        """POST /api/companion/update — 更新陪伴状态（体动/时间反馈）"""
        self._set_headers()
        openid = data.get('openid', 'default')
        feedback = data.get('feedback', {})

        try:
            sessions = getattr(self.__class__, '_companion_sessions', {})
            session = sessions.get(openid)
            if not session:
                self.wfile.write(json.dumps({'success': False, 'error': '没有活跃的陪伴会话'}, ensure_ascii=False).encode('utf-8'))
                return
            ...
            self.wfile.write(json.dumps({
                ...
                'next_text': action.get('text', ''),
                'phase': action.get('phase', ''),
                'duration_s': action.get('duration_s', 0),
    ...
    def _handle_companion_status(self, data):
        """POST /api/companion/status — 查询当前陪伴状态"""
        self._set_headers()
        openid = data.get('openid', 'default')
        ...
        try:
            ...
            session = sessions.get(openid)
            if not session:
                self.wfile.write(json.dumps({'active': False, 'message': '无活跃会话'}, ensure_ascii=False).encode('utf-8'))
                return
            ...
            self.wfile.write(json.dumps({
                ...
                'elapsed_text': '%d:%02d' % (int(elapsed)//60, int(elapsed)%60),
            }, ensure_ascii=False).encode('utf-8'))
        except Exception as e:
            self.wfile.write(json.dumps({'active': False, 'error': str(e)[:60]}).encode('utf-8'))

    def _handle_companion_stop(self, data):
        """POST /api/companion/stop — 停止陪伴"""
        self._set_headers()
        openid = data.get('openid', 'default')
        ...
        try:
            sessions = getattr(self.__class__, '_companion_sessions', {})
            session = sessions.pop(openid, None)

            # 也尝试通知 chat handler 上下文
            try:
                from body_context import report_body_event
                report_body_event(openid, 'companion_ended', {})
            except Exception:
                pass

            self.wfile.write(json.dumps({
                'success': True,
                'duration_s': round(session.get_elapsed(), 1) if session else 0,
                'message': '陪伴结束，晚安',
            }, ensure_ascii=False).encode('utf-8'))
        except Exception as e:
            self.wfile.write(json.dumps({'success': True, 'message': '已停止'}).encode('utf-8'))

    # ── AI 助眠音频 ──
    # ===== [乙醛酸启示③] 备用Plan B：API不可用时的降级推理链 =====
    # 当DeepSeek API超时/报错时，由_fallback_chain接管回复
    # 从EVIDENCE_BASE匹配关键词+返回预生成建议，不依赖外部API
    def _build_fallback_response(self, user_message: str) -> str:
        """纯本地降级回复：基于关键词匹配 + 预生成建议，不调任何API"""
        _sleep_issues = {
            '入睡困难': ('刺激控制疗法', '尝试只在困了才上床，不在床上做睡觉以外的事。'),
            '夜醒': ('睡眠限制疗法', '限制卧床时间到实际平均睡眠时间，逐步增加。'),
            '早醒': ('认知重构', '挑战"天亮了就该起床了"的固定思维，允许自己闭目养神。'),
            '焦虑': ('PMR', '从脚趾到头顶，逐一紧绷再放松每个肌肉群。'),
            '呼吸': ('4-7-8呼吸法', '吸气4秒-屏息7秒-呼气8秒，重复5次。'),
            '打鼾': ('OSA筛查', '建议就医进行睡眠呼吸监测，尝试侧卧睡眠。'),
            '噩梦': ('想象排练', '白天想象一个不同的结局反复排练。'),
            '疼痛': ('CBT-I适应', '疼痛时尝试聚焦呼吸而非疼痛本身。'),
        }
        msg_lower = user_message.lower()
        for keyword, (therapy, advice) in _sleep_issues.items():
            if keyword in msg_lower:
                return f'注意您提到{keyword}相关。推荐方案：{therapy}。{advice}（离线降级模式，建议网络恢复后获取更精准分析）'
        return '当前处于离线降级模式。建议：固定就寝时间、睡前1小时避电子设备、20分钟未入睡起床活动。记日志待分析。'
...
def _handle_world_step(self, data):
    ...
    try:
        ...
        # === 因果推理注入 ===
        causal = result.get('_causal_analysis', {})
        if causal.get('root_cause') and causal.get('explanation'):
            causal_path = causal.get('causal_path', {})
            top_chain = causal_path.get('chains', [{}])[0] if causal_path.get('chains') else {}
            result['action']['causal'] = {
                'root_cause': causal['root_cause_label'],
                'explanation': causal['explanation'],
                'suggestion': causal.get('action_suggestion', ''),
                'confidence': round(causal.get('confidence', 0), 2),
            }
            if top_chain:
                result['action']['causal']['top_chain'] = top_chain.get('cause', '')
...
# ============================================================
# CLEAN_DEPLOY_v3: 乙醛酸循环改造（2026-06-13）
# 启示一：加强复古路径（CBT-I降级模式）
# 启示三：应急推理通路（输入噪声检测→降级推理）
# 启示四：代谢签名检测（伪专业回复→自动降权）
# ============================================================

_METABOLIC_CACHE = {}  # 避免重复计算


def _inject_metabolic_audit(original_response: str, user_message: str,
                            profile: dict, quiet_pct: float = 0) -> str:
    """代谢签名检测（启示四）：审计回复站不站得住"""
    if not original_response or len(original_response) < 20:
        return original_response

    lower = original_response.lower()

    # 代谢签名：查看推理链类型
    has_numeric = bool(re.search(r'\d+%|\d+min|\d+ms|\d+bpm|\d+次', original_response))
    has_multi_option = '可能是' in original_response or '要么' in original_response or '情况一' in original_response
    has_analogy = ('就像' in lower or '正如' in lower or '类似于' in lower)
    has_single_story = ('原因是' in lower or ('结论' in lower and '可能' not in lower))

    # 高危模式：类比驱动+单一叙事=伪专业
    if has_analogy and has_single_story:
        flag = '⚠️ 代谢高危：类比+单一叙事'
        confidence_note = '（此结论基于类比推理，置信度估计40%以下，建议核实）'
        if not has_numeric:
            return original_response.rstrip('\n.。') + '。注意：' + confidence_note

    # 中危模式：有数字但无替代假设
    if has_numeric and not has_multi_option and has_single_story:
        confidence_note = '（此结论有其数据支撑，但未考虑替代解释，置信度约60%）'
        return original_response.rstrip('\n.。') + '。' + confidence_note

    return original_response


def _estimate_input_noise(user_message: str, profile: dict) -> dict:
    """应急推理通路（启示三）：评估输入噪声水平"""
    if not user_message:
    ...
    # 2. 矛盾信号
    contradictions = ['但是', '但是又', '有时候', '说不清', '混乱']
    if any(c in msg_lower for c in contradictions):
        score += 20
        reasons.append('含矛盾信号')

    # 3. 情绪标记→焦虑等情绪影响客观性
    emotion_signals = ['焦虑', '心烦', '失眠', '睡不着', '难受']
    if any(e in msg_lower for e in emotion_signals):
        score += 10

    # 4. 数值缺失→无具体数字
    has_num = bool(re.search(r'\d+', user_message))
    if not has_num:
        score += 15
        reasons.append('无量化数据')

    # 5. 历史数据方差（如果有）
    ab = profile.get('sleep_history', [])
    if len(ab) >= 3:
        vals = [s.get('total_min', 0) for s in ab[-5:] if s.get('total_min')]
        if vals and max(vals) - min(vals) > 120:
            score += 15
            reasons.append('体动历史波动大')
    ...
    return {
        ...
        'score': score,
        'reasons': reasons,
        'use_fallback': score >= 40,
        'confidence_bonus': -score * 0.5
    }


def _get_retro_fallback_response(user_message: str) -> str:
    """加强的复古路径（启示一）：不依赖AI，纯规则引擎"""
    msg_lower = user_message.lower()

    _sleep_issues = {
        '入睡困难': ('刺激控制疗法', '只在困了才上床。卧床20分钟未入睡→起床做放松活动，待有睡意再回床。'),
        '夜醒': ('睡眠限制疗法', '将卧床时间压缩到实际平均睡眠时长（误差≤30分钟），禁止白天小睡。'),
        '早醒': ('清晨光疗法', '起床后立即暴露于强光（1200lux以上）15分钟，固定起床时间±30分钟。'),
        '焦虑': ('渐进肌肉放松', '从脚趾→小腿→大腿→腹部→胸部→手→面部，每组绷紧7秒后缓缓放松15秒。'),
        '呼吸': ('4-7-8呼吸法', '吸气4秒→屏息7秒→呼气8秒为一轮。每晚至少5轮，每分钟<6次呼吸。'),
        '惊醒': ('认知重构', '惊醒后标记为"无害生理事件"，不赋予其灾难性解读。'),
        '噩梦': ('意象排练疗法', '白天想象一个不同结局，反复排练7天。'),
        '打鼾': ('体位疗法', '侧卧睡眠，使用止鼾枕/网球衣。如夜间呼吸中断≥10秒需就医。'),
        '疼痛': ('聚焦呼吸', '将注意力放在呼吸的进出上，而非疼痛部位。疼痛高峰→呼气延长。'),
        '昼夜': ('生物钟重置', '固定起床时间（±15分钟），清晨急速暴露，傍晚避强光。'),
    }

    for keyword, (therapy, advice) in _sleep_issues.items():
        if keyword in msg_lower:
            return (f'【离线降级·规则引擎】'
                    f'\n检测到关键词「{keyword}」，匹配方案：{therapy}。'
                    f'\n{advice}'
                    f'\n（此回复基于CBT-I标准化协议，置信度约75%。'
                    f'网络恢复后建议获取AI个性化分析。）')
    ...
    return ('【离线降级·规则引擎】'
            '\n当前无法调用推理模型。'
            '\n基础建议：'
            '\n1. 固定就寝时间(22:30-23:00) 和起床时间(06:00-06:30)'
            '\n2. 睡前1小时避电子设备（蓝光抑制褪黑素）'
            '\n3. 20分钟未入睡→起床活动，待有睡意再回床'
            '\n记录未来3晚的睡眠日志，网络恢复后可自动分析。')


def _patch_build_fallback():
    """用加强版替换原版（启示一）"""
    old_method = ProxyHandler._build_fallback_response
    new_method = _get_retro_fallback_response
    # 不破坏原有调用方式，直接在方法中注入
    ProxyHandler._original_fallback = old_method

    def patched_fallback(self, user_message):
        # 输入噪声检测
        profile = {}
        try:
            profile = _load_user_profile('default')
        except:
            pass
        noise = _estimate_input_noise(user_message, profile)
        if noise['level'] == 'high' and noise['use_fallback']:
            return _get_retro_fallback_response(user_message)
        return old_method(self, user_message)

    ProxyHandler._build_fallback_response = patched_fallback


def _patch_chat_metabolic():
    """在_handle_chat输出前注射代谢签名检测"""
    old_handler = ProxyHandler._handle_chat

    def patched_handler(self, data):
        result = old_handler(self, data)
        # 代谢签名只在有回复时启用
        try:
            if result and isinstance(result, str):
                ...
                try:
                    ...
                    pass
                # 估计噪声
                noise = _estimate_input_noise(user_msg, profile)
                if noise['level'] == 'high':
                    # 高噪声时加前缀提示
                    noise_msg = ('【⚠️ 你的输入波动较大（含矛盾信号/数据不足），'
                                 '结论置信度约{:.0f}%。建议补充具体数据获取更准确分析。】\n'.format(
                                 max(20, 80 - noise['score'])))
                    result = noise_msg + result
        except Exception:
            pass
        return result

    ProxyHandler._handle_chat = patched_handler


# 应用补丁
_patch_build_fallback()
_patch_chat_metabolic()
print('[乙醛酸循环改造] 三条通路已注入：复古路径(E1) + 应急通路(E3) + 代谢签名(E4)')
