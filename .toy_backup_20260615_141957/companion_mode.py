...
def start_companion(openid, protocol='4-7-8', message=''):
    ...
    if _tier == 1:
        ...
        protocol = suggest_strategy(openid)  # 跨夜学习选择最优协议
...
def update_companion(openid, feedback):
    ...
    if result.get('state') == 'EXIT' or result.get('action') == 'exit':
        if openid in _active_sessions:
            record_night_outcome(openid,
                protocol_used=session.protocol_name,
                reset_count=session.reset_count,
                total_duration_s=session.get_elapsed(),
                fell_asleep=result.get('forecast', {}).get('will_sleep_soon', False) or session.state == 'RELEASE')
            del _active_sessions[openid]
            _report_companion_exit(openid)
...
# ===== [v5.2 SenNet] 跨夜策略选择器 =====
_STRATEGY_HISTORY_PATH = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'frontier_data', 'companion_strategy.json')
os.makedirs(os.path.dirname(_STRATEGY_HISTORY_PATH), exist_ok=True)
