...
class Chronobiologist:
    """昼夜节律专家 — 多模态：历史入睡时间序列 + 单日数据回退

    多模态感知分裂(2026-07-08):
    - 如果有 openid → 从 SQLite 拉最近7天入睡时间序列
    - 算出：均值、标准差(节律稳定性)、趋势、社会时差
    - 有序列时替代单点bedtime判断，避免同源输入
    - 无序列/数据不足3天 → 回退到单日bedtime逻辑
    """
    ...
    def analyze(self, data: Dict, peer_findings: Dict = None, evidence: list = None) -> Dict:
        ...
        if openid and openid != 'default':
            try:
                from sqlite_db import load_profile as _load_sqlite_profile
                _sp = _load_sqlite_profile(openid)
                if _sp and isinstance(_sp, dict):
                    _history = _sp.get('history', [])
                    if len(_history) >= 3:
                        # 提取最近7天的睡觉小时
                        _bed_hours = []
                        _wake_hours = []
                        for _h in _history[-7:]:
                            _b = self._parse_hour(_h.get('bedtime', ''))
                            _w = self._parse_hour(_h.get('wake_time', ''))
                            if _b is not None:
                                _bed_hours.append((_h.get('date', ''), _b))
                            if _w is not None:
                                _wake_hours.append((_h.get('date', ''), _w))

                        if len(_bed_hours) >= 3:
                            _bed_vals = [b for _, b in _bed_hours]
                            _bed_mean = sum(_bed_vals) / len(_bed_vals)
                            _bed_std = (sum((v - _bed_mean)**2 for v in _bed_vals) / len(_bed_vals)) ** 0.5
                            _bed_trend = _bed_vals[-1] - _bed_vals[0] if len(_bed_vals) >= 3 else 0

                            # 基于序列的节律评估
                            if _bed_std > 1.5:
                                penalty += 0.20
                                findings.append(f'入睡时间波动大(标准差{_bed_std:.1f}h),节律稳定性差')
                                risk_flags.append({'type':'节律','detail':'入睡时间高度不规律','risk_level':'high'})
                            elif _bed_std > 1.0:
                                penalty += 0.12
                                findings.append(f'入睡时间有波动(标准差{_bed_std:.1f}h),建议固定就寝时间')
                                risk_flags.append({'type':'节律','detail':'入睡时间不规律','risk_level':'medium'})
                            else:
                                findings.append(f'入睡时间较规律(标准差{_bed_std:.1f}h),节律稳定性良好')

                            # 趋势检测
                            if _bed_trend > 1.0:
                                penalty += 0.10
                                findings.append(f'入睡时间呈推迟趋势(7天内推迟{_bed_trend:.1f}h),警惕节律漂移')
                            elif _bed_trend < -1.0:
                                findings.append(f'入睡时间呈提前趋势(7天内提前{abs(_bed_trend):.1f}h),积极变化')

                            # 社会时差(周末vs工作日)
                            _weekday_beds = [v for d, v in _bed_hours if d and len(d) >= 5 and int(d[-2:]) % 7 <= 1]
                            _weekend_beds = [v for d, v in _bed_hours if d and len(d) >= 5 and int(d[-2:]) % 7 >= 5]
                            if len(_weekday_beds) >= 2 and len(_weekend_beds) >= 1:
                                _wd_mean = sum(_weekday_beds) / len(_weekday_beds)
                                _we_mean = sum(_weekend_beds) / len(_weekend_beds)
                                _social_jetlag = _we_mean - _wd_mean
                                if _social_jetlag > 1.5:
                                    penalty += 0.12
                                    findings.append(f'社会时差{_social_jetlag:.1f}h(周末比工作日晚睡),节律冲突显著')
                                    risk_flags.append({'type':'节律','detail':f'社会时差{_social_jetlag:.1f}h','risk_level':'medium'})
                                elif _social_jetlag > 0.5:
                                    penalty += 0.05
                                    findings.append(f'轻微社会时差{_social_jetlag:.1f}h')

                            # 基于序列平均计算chronotype（替代单点估算）
                            if _bed_mean >= 24 or _bed_mean < 3:
                                chronotype = 'evening'
                            elif _bed_mean >= 23:
                            ...
                            findings.append(f"生物钟类型(基于7天序列): {chrono_descs.get(chronotype, chronotype)}")

                            # 序列通道的 narrative
                            ch_narr = []
                            ch_narr.append(f"基于最近{len(_bed_hours)}天入睡时间分析")
                            if _bed_std > 1.0:
                                ch_narr.append(f"入睡时间波动{_bed_std:.1f}小时，节律稳定性需改善")
                            else:
                                ch_narr.append(f"入睡时间波动仅{_bed_std:.1f}小时，节律稳定")
                            if _bed_trend > 1.0:
                                ch_narr.append(f"趋势显示入睡时间正在推迟")
                            if '社会时差' in str(findings):
                                ch_narr.append(f"周末-工作日入睡时间差{_social_jetlag:.1f}小时，存在社会时差")
                            ch_narr.append(f"生物钟类型为{chrono_descs.get(chronotype, '未知')}")
                            narrative = "。".join([p for p in ch_narr if p]) + "。"
                            score = max(0.1, min(1.0, 0.85 - penalty))
                            return {
                                'score': round(score, 2), 'findings': findings, 'risk_flags': risk_flags,
                                'chronotype': chronotype, 'confidence': 0.80, 'specialty': '时间生物学(序列)',
                                'narrative': narrative,
                            }
            except Exception:
                pass  # 序列通道失败, 回退到单日逻辑

        # ===== 回退：单日bedtime通道（原逻辑精简版）=====
        ...
        if bed_hour is not None:
            if bed_hour >= 24 or bed_hour < 3:
                findings.append(f"入睡{bed_hour:.0f}:00偏离褪黑素窗口");
                penalty += 0.15
            elif bed_hour >= 23:
                findings.append(f"入睡{bed_hour:.0f}:00略晚于最佳窗");
                penalty += 0.08
            if bed_hour >= 24 and sleep_latency > 30:
                findings.append("晚睡+入睡困难，疑似DSPD[昼夜节律失调]");
                risk_flags.append("昼夜节律失调")
        if wake_hour is not None:
            if wake_hour < 5:
                findings.append(f"醒来{wake_hour:.0f}:00过早觉醒");
                penalty += 0.08
        if bed_hour is not None and wake_hour is not None:
            dur = (wake_hour + 24 - bed_hour) % 24
            if dur > 10:
                findings.append(f"卧床{dur:.0f}h过长");
                penalty += 0.08
        if screen_time > 60:
            findings.append(f"睡前屏幕{screen_time}min抑制褪黑素");
            penalty += 0.10
        elif screen_time > 30:
            findings.append("睡前屏幕>30min影响褪黑素[Chang 2015, PNAS]");
            penalty += 0.05
        if peer_findings:
            cp = peer_findings.get('ClinicalPsychologist', {})
...
class CardiacRiskMonitor:
    """心血管风险评估 — 多模态：心率(hr)时序数据 + 文本症状关键词

    多模态感知分裂(2026-07-08):
    - 如果有 hr 数值数据 → 直接基于心率评估（心率均值、静息心率偏差、波动标准差）
    - 如果无 hr → 回退到文本关键词检测
    - 有hr时完全替代关键词检测，避免同源输入
    """
    ...
    def analyze(self, data: Dict, peer_findings: Dict = None, evidence: list = None) -> Dict:
        ...
        # ===== 多模态感知：hr 数值通道 =====
        hr = data.get('hr')
        if hr is not None and isinstance(hr, (int, float)) and hr > 20:
            # 有实时心率数据 → 用数值通道评估
            # 参考值：静息心率 60-100bpm，夜间深度睡眠可降至 40-60
            if hr < 40:
                score = 0.25
                confidence = 0.7
                findings.append(f'夜间心率{int(hr)}bpm(显著过缓),需排查窦性心动过缓或药物影响')
                risk_flags.append({'type':'心血管','detail':f'心率过缓{int(hr)}bpm','risk_level':'high'})
            elif hr < 50:
                score = 0.40
                confidence = 0.65
                findings.append(f'夜间心率{int(hr)}bpm(偏缓),运动员可接受,普通人群建议复核')
                risk_flags.append({'type':'心血管','detail':f'心率偏缓{int(hr)}bpm','risk_level':'medium'})
            elif hr > 100:
                score = 0.30
                confidence = 0.75
                findings.append(f'夜间心率{int(hr)}bpm(显著过速),提示交感神经过度激活或焦虑状态')
                risk_flags.append({'type':'心血管','detail':f'心率过速{int(hr)}bpm','risk_level':'high'})
            elif hr > 85:
                score = 0.45
                confidence = 0.65
                findings.append(f'夜间心率{int(hr)}bpm(偏高),可能与压力/咖啡因/睡前运动相关')
                risk_flags.append({'type':'心血管','detail':f'心率偏高{int(hr)}bpm','risk_level':'medium'})
            else:
                # 正常范围 50-85
                score = 0.75
                confidence = 0.6
                findings.append(f'夜间心率{int(hr)}bpm(正常范围),自主神经调节功能良好')

            # 如果有历史心率数据,可做偏差分析
            # （预留：后续从 profile 拉最近7天均值做偏离检测）

            # hr通道也构建narrative,供return统一使用
            card_narr = []
            if score < 0.5:
                card_narr.append(f"心率{int(hr)}bpm偏离正常范围,建议关注")
            else:
                card_narr.append(f"心率{int(hr)}bpm在正常范围")
            # age+bmi共享逻辑在try块外处理,这里先构建基础narrative
            narrative = "。".join([p for p in card_narr if p]) + "。"
            detected = {'palpitations': False, 'breathless': False, 'sweats': False, 'snoring': False}  # 占位,供return引用
        else:
            # ===== 无 hr 数据 → 回退到文本关键词通道 =====
            message = (data.get('raw_text', '') or '').lower()
            score = 0.5
            ...
            if not any(detected.values()):
                findings.append('未检测到明显心血管夜间风险信号(文本通道)')
        # ===== 心血管叙述 =====
        card_narr = []
        if detected['palpitations']:
...
class WorldModelEngine:
    ...
    def comprehensive_analysis(self, sleep_data: Dict, today_str: str = '', profile: dict = None) -> Dict:
        ...
        # ═══ 专家声誉追踪：检测作弊/搭便车/退化 ═══
        # 《狡猾的细胞》启示：演化系统中欺骗是固有属性，
        # 需要"免疫监视"检测异常行为并降权
        # 声誉指标: unique_insight_ratio, divergence_from_mean, response_complexity
        try:
            reputation_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'expert_reputation.json')
            expert_reputation = {}
            if os.path.exists(reputation_path):
                with open(reputation_path, 'r', encoding='utf-8') as _f:
                    expert_reputation = json.load(_f)

            # 收集本轮各专家的表现
            current_round = round2
            # 哪些专家的发现跟其他人高度相似（搭便车信号）
            all_finding_sets = {}
            for _rn, _rr in current_round.items():
                _raw = ' '.join(_rr.get('findings', []))
                all_finding_sets[_rn] = (_raw, _rr.get('score', 0.5), _rr.get('confidence', 0.5))

            # 计算两两相似度，检测"克隆体"
            _expert_names_all = list(current_round.keys())
            for i in range(len(_expert_names_all)):
                for j in range(i+1, len(_expert_names_all)):
                    _a, _b = _expert_names_all[i], _expert_names_all[j]
                    _text_a = all_finding_sets.get(_a, ('',0,0))[0]
                    _text_b = all_finding_sets.get(_b, ('',0,0))[0]
                    if not _text_a or not _text_b:
                        continue
                    # Token-level Jaccard: 用共用词做粗略相似度
                    _tokens_a = set(_text_a.replace('【',' ').replace('】',' ').replace('，',' ').replace('。',' ').split())
                    _tokens_b = set(_text_b.replace('【',' ').replace('】',' ').replace('，',' ').replace('。',' ').split())
                    if not _tokens_a or not _tokens_b:
                        continue
                    _jaccard = len(_tokens_a & _tokens_b) / max(len(_tokens_a | _tokens_b), 1)
                    if _jaccard > 0.85:  # 高度相似 → 搭便车标记
                        # 记录到声誉：搭便车者降权，独立者加权
                        for _target in [_a, _b]:
                            _rep = expert_reputation.setdefault(_target, {
                                'total_runs': 0, 'free_ride_hits': 0, 'unique_insight_count': 0,
                                'avg_score': 0.5, 'last_score': 0.5, 'recent_latency': [],
                            })
                            _rep['free_ride_hits'] = _rep.get('free_ride_hits', 0) + 1

            # 更新每个专家的声誉数据
            round2_mean_score = sum(_rr.get('score', 0.5) for _rr in current_round.values()) / max(len(current_round), 1)
            for _rn, _rr in current_round.items():
                _rep = expert_reputation.setdefault(_rn, {
                    'total_runs': 0, 'free_ride_hits': 0, 'unique_insight_count': 0,
                    'avg_score': 0.5, 'last_score': 0.5, 'recent_latency': [],
                })
                _rep['total_runs'] = _rep.get('total_runs', 0) + 1
                _rep['last_score'] = _rr.get('score', 0.5)
                _prev_avg = _rep.get('avg_score', 0.5)
                n = _rep['total_runs']
                _rep['avg_score'] = _prev_avg + (_rr.get('score', 0.5) - _prev_avg) / n

                # 独立见解：分数偏离均值且发现独特 → 加分
                _divergence = abs(_rr.get('score', 0.5) - round2_mean_score)
                if _divergence > 0.1:  # 偏离均值超过10% → 有独立见解
                    _rep['unique_insight_count'] = _rep.get('unique_insight_count', 0) + 1

            # 计算声誉系数（0.5~1.5，乘到confidence上）
            reputation_bonus = {}
            for _rn in current_round:
                _rep = expert_reputation.get(_rn, {})
                _total = _rep.get('total_runs', 1)
                _hits = _rep.get('free_ride_hits', 0)
                _unique = _rep.get('unique_insight_count', 0)
                # 基本权重 1.0
                _bonus = 1.0
                # 搭便车惩罚：每10次中多一次搭便车就-0.05
                if _total > 3:
                    _freeload_ratio = _hits / _total
                    _bonus -= _freeload_ratio * 0.3  # 最多-0.3
                # 独立见解奖励
                if _total > 3:
                    _unique_ratio = _unique / _total
                    _bonus += _unique_ratio * 0.2  # 最多+0.2
                _bonus = max(0.5, min(1.5, _bonus))
                reputation_bonus[_rn] = _bonus

            # 应用声誉系数到confidence
            for _rn in current_round:
                if _rn in reputation_bonus:
                    _orig_conf = current_round[_rn].get('confidence', 0.5)
                    current_round[_rn]['confidence'] = round(
                        max(0.05, min(0.95, _orig_conf * reputation_bonus[_rn])), 3
                    )
                    current_round[_rn]['_reputation_bonus'] = round(reputation_bonus[_rn], 3)

            # 持久化声誉数据（只保留最近100次运行）
            for _rn in expert_reputation:
                _rep = expert_reputation[_rn]
                if _rep.get('total_runs', 0) > 100:
                    _rep['free_ride_hits'] = max(0, _rep.get('free_ride_hits', 0) // 2)
                    _rep['unique_insight_count'] = max(0, _rep.get('unique_insight_count', 0) // 2)
                    _rep['total_runs'] = 50
            with open(reputation_path, 'w', encoding='utf-8') as _f:
                json.dump(expert_reputation, _f, ensure_ascii=False, indent=2)
        ...
        # ═══ 不确定性校准 (UA-ChatDev, 2026-07-06) ═══
        # 用校准后的 confidence 替代固定值, 提升数据充分专家的权重
        try:
            from dev_tools.uncertainty_calibration import UncertaintyCalibrator
            _profile_lc = (profile or {}).get('_learning_context', {})
            _pb = _profile_lc.get('personal_bias', (0, 0, 0, 0.3))
            _avg_err = _pb[3] if isinstance(_pb, (list, tuple)) and len(_pb) >= 4 else 0.3
            _lc_conf = _profile_lc.get('learning_confidence', 'low')
            _sample_count = len(profile.get('feedback', [])) if isinstance(profile, dict) else 0

            for _name in list(round2.keys()):
                _expert = round2[_name]
                _data_fields = sum(1 for k in ['bedtime','wake_time','sleep_latency','awake_times','total_duration','stress_level'] if data.get(k))
                # 方差: 如果有历史评分, 估一个
                _var = 0.15
                _peer_count = sum(1 for n in round2 if n != _name)
                _cal_conf = UncertaintyCalibrator.calibrate(
                    _name, _data_fields, _var, _peer_count, _avg_err, _lc_conf, _sample_count
                )
                _expert['confidence'] = round((_expert.get('confidence', 0.5) + _cal_conf) / 2, 2)
        except Exception as _uce:
            pass  # 校准失败不中断, 回退到原始confidence

        # ═══ 多样性抗退化过滤（2026-07-08 狡猾的细胞启发）═══
        # 检测专家集体"克隆体"退化：所有专家收敛到几乎一致的评分/发现
        # → 通过置信度惩罚拉低过高一致性，迫使多样性
        try:
            _round2_scores = [r.get('score', 0.5) for r in round2.values()]
            _round2_confs = [r.get('confidence', 0.5) for r in round2.values()]
            if len(_round2_scores) >= 5:
                _score_std = (sum((s - sum(_round2_scores)/len(_round2_scores))**2 for s in _round2_scores) / len(_round2_scores)) ** 0.5
                # 评分标准差 < 0.05 且 置信度都 > 0.7 → 过度一致信号
                if _score_std < 0.05 and all(c > 0.7 for c in _round2_confs):
                    _conf_mean = sum(_round2_confs) / len(_round2_confs)
                    for _rn in round2:
                        _orig_conf = round2[_rn].get('confidence', 0.5)
                        # 对每个专家注入个体差异惩罚：越靠近均值越罚
                        _deviation = abs(round2[_rn].get('score', 0.5) - 0.6)
                        _diversity_penalty = 1.0 - _deviation * 0.3  # 偏离均值越少惩罚越重
                        round2[_rn]['confidence'] = round(max(0.1, _orig_conf * _diversity_penalty), 3)
                        round2[_rn]['_diversity_penalty'] = round(_diversity_penalty, 3)
        except Exception as _dive:
            pass

        # ═══ 瑞奇流曲率感知调整（2026-07-17 Ricci Flow 启示）═══
        # 在多样性抗退化之后、加权汇总之前，用曲率做精细置信度调整
        try:
            from ricci_flow import RicciFlowCurvature, CurvatureLogger
            _ricci = getattr(self, '_ricci_flow', None)
            if _ricci is None:
                _ricci = RicciFlowCurvature()
                self._ricci_flow = _ricci
                self._ricci_logger = CurvatureLogger()
            round2, _curv_map = _ricci.adjust(round2)
            self._ricci_logger.log(_curv_map)
            # 注入曲率信息到data供后续evograd使用
            if isinstance(data, dict):
                data['_ricci_curvatures'] = {n: v['combined_curvature'] for n, v in _curv_map.items()}
        except ImportError:
            pass  # ricci_flow.py 不存在时跳过
        ...
        # ═══ LightGBM 校正: 替代简单加权平均 ═══
        try:
            from dev_tools.ml_correction import ExpertCorrector
            _corrector = ExpertCorrector()
            _ml_result = _corrector.correct(round2)
            if "LightGBM校正" in _ml_result.get("note", ""):
                weighted_score = _ml_result["score"]
        except Exception:
            pass

        # ═══ 第三层: 冲突惩罚 (从 perceiver 融合结果中提取) ═══
        try:
            _fusion = data.get('perceiver_fusion', {}) if isinstance(data, dict) else {}
            _conflict = _fusion.get('conflict_penalty', 0.0)
            if _conflict > 0.0:
                weighted_score = round(weighted_score * (1.0 - _conflict), 4)
        except Exception:
            pass

        # ═══ 第四层: 反思循环 - 回访分歧专家 ═══
        # 《狡猾的细胞》"带瘤生存"启示：灰色容忍区内的分歧视为健康多样性，
        # 而非系统缺陷。只在有人踩到极端区域(score<0.3或>0.7)时才收窄。
        # 原理: 当专家间分歧大时, top-3高置信专家重新评分
        # 分歧标准: top-3的评分标准差 > 0.15
        try:
            _scores_arr = [r.get("score", 0.5) for r in round2.values()]
            _confs_arr = [r.get("confidence", 0.5) for r in round2.values()]
            if len(_scores_arr) >= 3:
                _mean_s = sum(_scores_arr) / len(_scores_arr)
                _std_s = (sum((s - _mean_s)**2 for s in _scores_arr) / len(_scores_arr)) ** 0.5
                if _std_s > 0.15:
                    # 灰色容忍区检测：分歧大但所有人都在0.3-0.7之间 = 健康多样性
                    _all_in_gray_zone = all(0.3 <= s <= 0.7 for s in _scores_arr)
                    if not _all_in_gray_zone:
                        # 有人踩到极端区域(score<0.3或>0.7) → 回访修正
                        # 找top-3高置信专家回访
                        _expert_pairs = [(r.get("confidence", 0), n) for n, r in round2.items()]
                        _expert_pairs.sort(reverse=True)
                        _top3 = _expert_pairs[:3]
                        _revisit_scores = []
                        for _conf, _name in _top3:
                            _r = round2.get(_name, {})
                            # 回访 = 原评分 + 取整修正 (模拟二次思考)
                            _orig = _r.get("score", 0.5)
                            # 回访修正: 向均值收窄 (分歧越大收窄越多)
                            _pull = 0.1  # 10%回缩
                            _revisit = _orig * (1 - _pull) + _mean_s * _pull
                            _revisit_scores.append(_revisit)
                        # 回访加权平均
                        _revisit_w = sum(_c for _c, _ in _top3)
                        _revisit_avg = sum(_rs * _c for _rs, (_c, _) in zip(_revisit_scores, _top3)) / max(_revisit_w, 0.01)
                        # 混合: 50%原始 + 50%回访
                        weighted_score = round(weighted_score * 0.5 + _revisit_avg * 0.5, 4)
        except Exception:
            pass
        ...
        result = {
            ...
            'insights': {
                ...
                'primary_focus': self._build_actionable_takeaway(round2, all_findings, all_risks, sleep_data=data),
        ...
        for _en, _er in round2.items():
            ...
            if _risks:
                _first_risk = _risks[0]
                if isinstance(_first_risk, dict):
                    _risk_text = _first_risk.get('detail', str(_first_risk))[:40]
                else:
                    _risk_text = str(_first_risk)[:40]
                _parts.append('风险: ' + _risk_text)
                if len(_risks) > 1:
        ...
        # 让用户看到"为什么是这个分数"
        try:
            from dev_tools.feature_extractor import extract_base_features as _extract_base
            _fb_features = _extract_base(sleep_data)
            _feat_names = ["wm_score", "latency", "awake", "duration", "stress", "pain",
                           "coef_wm", "coef_lat", "coef_awake", "coef_dur", "coef_stress", "coef_pain",
                           "happy_ratio", "pain_penalty", "freq_awake"]
            _fb_dict = {_feat_names[i]: _fb_features[i] for i in range(min(len(_feat_names), len(_fb_features)))}
            # 分值贡献: 各专家score vs 全局均值
            _expert_scores = {n: r.get("score", 0.5) for n, r in round2.items()}
            _mean_expert = sum(_expert_scores.values()) / max(len(_expert_scores), 1)
            _attribution = {}
            for _n, _s in _expert_scores.items():
                _delta = _s - _mean_expert
                _spec = round2.get(_n, {}).get("specialty", _n)
                _attribution[_n] = {
                    "specialty": _spec,
                    "score": round(_s, 3),
                    "delta": round(_delta, 3),
                    "direction": "up" if _delta > 0.05 else ("down" if _delta < -0.05 else "neutral"),
                }
            result["attribution"] = {
                "experts": _attribution,
                "mean_expert_score": round(_mean_expert, 3),
                "final_score": round(weighted_score, 3),
                "top_positive": sorted([(n, a) for n, a in _attribution.items() if a["direction"] == "up"],
                                      key=lambda x: -x[1]["delta"])[:2],
                "top_negative": sorted([(n, a) for n, a in _attribution.items() if a["direction"] == "down"],
                                      key=lambda x: x[1]["delta"])[:2],
            }
        except Exception as _attr_e:
            result["attribution"] = {"note": f"归因失败: {str(_attr_e)[:40]}"}

        # ═══ Constitutional AI: 自约束过滤 ═══
        try:
            from dev_tools.constitutional_filter import filter_result as _con_filter
            result = _con_filter(result)
        except Exception as _con_e:
            result["_constitutional"] = {"safe": True, "errors": str(_con_e)[:40]}

        # ═══ 免疫探针：独立统计健康检查（2026-07-08）═══
        # 《狡猾的细胞》"隐蔽后门→免疫探针"映射
        # 不依赖DeepSeek，纯数学检测系统性异常
        # 结果写入日志（不阻塞返回）
        try:
            _probe = {}
            # 1. 专家评分熵：一致性死锁检测
            _scores = [r.get('score', 0.5) for r in round2.values()]
            if len(_scores) >= 5:
                _bins = [0] * 5  # 0-0.2, 0.2-0.4, 0.4-0.6, 0.6-0.8, 0.8-1.0
                for s in _scores:
                    _bins[min(int(s * 5), 4)] += 1
                _probs = [b / len(_scores) for b in _bins]
                _entropy = -sum(p * (p and __import__('math').log(p)) for p in _probs)
                _probe['entropy'] = round(_entropy, 3)
                if _entropy < 0.5:
                    _probe['warn'] = 'entropy_too_low'
                    print(f'[Probe] WARN: 专家评分熵{_entropy:.3f}<0.5, 可能一致性死锁')
                elif _entropy > 2.0:
                    _probe['warn'] = 'entropy_too_high'
                    print(f'[Probe] WARN: 专家评分熵{_entropy:.3f}>2.0, 可能专家分裂')

            # 2. 置信度均值：数据充分性检查
            _confs = [r.get('confidence', 0) for r in round2.values() if r.get('confidence')]
            if _confs:
                _avg_conf = sum(_confs) / len(_confs)
                _probe['avg_confidence'] = round(_avg_conf, 3)
                if _avg_conf < 0.25:
                    _probe['warn'] = 'confidence_too_low'
                    print(f'[Probe] WARN: 平均置信度{_avg_conf:.3f}<0.25, 数据严重不足')

            # 3. 评分全局偏离检测（需历史均值）
            _curr_avg = sum(_scores) / max(len(_scores), 1)
            _probe['current_avg_score'] = round(_curr_avg, 3)
            # 与最近3次的均值比较（如果有profile）
            if profile and isinstance(profile, dict):
                _probe_history = profile.get('_probe_history', [])
                if len(_probe_history) >= 3:
                    _hist_avg = sum(_probe_history[-3:]) / 3
                    _deviation = _curr_avg - _hist_avg
                    _probe['score_deviation'] = round(_deviation, 3)
                    if abs(_deviation) > 0.25:
                        _probe['warn'] = 'score_abnormal_deviation'
                        print(f'[Probe] WARN: 评分均值偏离历史{_deviation:.3f}, 可能DeepSeek异常')

            # 4. 输出完整性检查
            _expected_keys = {'total_score', 'quality', 'analysis', 'insights', 'action_plan'}
            _missing = _expected_keys - set(result.keys())
            if _missing:
                _probe['missing_keys'] = list(_missing)
                _probe['warn'] = 'incomplete_output'

            result['_health_probe'] = _probe

            # 写入 probe_history
            if profile and isinstance(profile, dict):
                _ph = profile.setdefault('_probe_history', [])
                _ph.append(_curr_avg)
                if len(_ph) > 30:
                    _ph.pop(0)
        except Exception as _probe_e:
            result['_health_probe'] = {'error': str(_probe_e)[:60]}

        # ═══ 偏离声音采集：作弊作为创新引擎 ═══
        # 不修改任何评分，只记录"偏离共识但置信度高"的专家意见
        # 让极端的正确声音不被系统过滤掉，留待审计追溯
        try:
            _divergent = []
            _r2_scores = [r.get('score', 0.5) for r in round2.values()]
            _r2_mean = sum(_r2_scores) / max(len(_r2_scores), 1)
            for _rn in round2:
                _r = round2[_rn]
                _dev = abs(_r.get('score', 0.5) - _r2_mean)
                if _dev > 0.2 and _r.get('confidence', 0) >= 0.5:
                    _keyfinding = (_r.get('findings', []) or [''])[0][:80]
                    _divergent.append({
                        'expert': _rn,
                        'specialty': _r.get('specialty', _rn),
                        'score': round(_r.get('score', 0.5), 3),
                        'consensus_mean': round(_r2_mean, 3),
                        'deviation': round(_dev, 3),
                        'confidence': round(_r.get('confidence', 0.5), 3),
                        'key_finding': _keyfinding,
                    })
            if _divergent:
                result['_divergent_voices'] = _divergent
                print(f'[Divergent] {len(_divergent)}个专家偏离共识: {", ".join(v["expert"] for v in _divergent)}')
        except Exception:
            pass
    ...
    def _build_perceiver(profile):
        ...
        try:
            ...
            # A/B实验: JEPA vs 标准fuse (2026-07-06)
            # 用户按 openid hash 分流, 50%标准版 50%JEPA
            _exp_group = "control"  # 默认对照组
            try:
                import json as _j, hashlib as _h
                _cal = _j.load(open(r'D:\AISleepGen_Optimized\data\calibration.json', 'r', encoding='utf-8'))
                _exp_config = _cal.get('_experiment_jepa', {})
                if _exp_config.get('enabled', False):
                    # A/B 分流: 按 profile 键 hash → 50% jepa / 50% control
                    _user_key = None
                    if isinstance(profile, dict):
                        _user_key = profile.get('openid') or profile.get('user_id') or profile.get('username', '')
                    if not _user_key:
                        _user_key = str(id(profile))
                    _hash = int(_h.md5(str(_user_key).encode()).hexdigest(), 16)
                    _exp_group = "jepa" if (_hash % 100) < 50 else "control"
                    # 把分组写回 profile 供下游埋点
                    if isinstance(profile, dict):
                        profile['_experiment_group'] = _exp_group
            except:
                _exp_group = "control"
            _use_jepa = (_exp_group == "jepa")

            result = fuse_modalities(texts, scores, dims, use_jepa=_use_jepa)
            result['experiment_group'] = _exp_group  # 给下游埋点用
    ...
    @staticmethod
    def _build_actionable_takeaway(round2, all_findings, all_risks, sleep_data=None):
        ...
        Args:
            ...
            all_findings: 所有发现
            all_risks: 所有风险
            sleep_data: 原始输入数据（含 feeling、indicators 等）
        """
        # ═══ 安全红线：用药相关 ═══
        if sleep_data:
            _message = sleep_data.get('feeling', '') or ''
            _indicators = sleep_data.get('indicators', []) or []
            # 用药关键词检测（直接扫描用户消息 + indicators）
            _med_kw = ['安眠药', '安眠', '安定', '安定', '佐匹克隆', '艾司唑仑', 
                       '褪黑素', '加大剂量', '加量', '多吃', '加倍', '双倍']
            _found_med = [kw for kw in _med_kw if kw in _message]
            if 'medication' in _indicators or _found_med:
                return ("不建议自行调整药物剂量。若安眠药效果不佳，请咨询开具处方的医生，"
                       "由医生评估是否需要调整方案。擅自加量可能导致副作用加重或药物依赖风险。")
...
def _update_personal_learning_context(profile, result):
    ...
    # ═══ 自噬检查点：《狡猾的细胞》"算法衰老→定期回滚" ═══
    # 如果有在线学习权重，检查是否出现持续退化：
    # 连续 N 次评分下降 → 回退到 3 次前的 snapshot
    # 相当于生物体的自噬——清除累积的错误表征，重置到健康状态
    try:
        _old_weights = lc.get('personal_weights', {})
        if _old_weights and _old_weights != dict(_DEFAULT_WEIGHTS):
            # 取最近 7 天的 total_score 看趋势
            _recent_totals = [h.get('total_score', 50) for h in history[-8:-1]]  # 不含当前
            if len(_recent_totals) >= 5:
                _decline_count = sum(1 for i in range(1, len(_recent_totals)) if _recent_totals[i] < _recent_totals[i-1])
                _decline_ratio = _decline_count / (len(_recent_totals) - 1)
                if _decline_ratio > 0.75:
                    # 持续退化(>3/4的时间都在下降) → 回退到 3 次前的 snapshot
                    _snapshot_3_ago = lc.get('_snapshot_rollback')
                    if _snapshot_3_ago:
                        weights = dict(_snapshot_3_ago)
                        lc['_autophagy_triggered'] = True
                        lc['_autophagy_at'] = datetime.now().isoformat()
                        print(f'[Autophagy] 自噬触发：连续退化率{_decline_ratio:.0%}, 已回退到上次稳定快照')
                    else:
                        # 没有rollback点, 至少保留当前权重作为未来的rollback目标
                        lc['_snapshot_rollback'] = dict(_old_weights)
                else:
                    # 正常情况：更新rollback snapshot
                    lc['_snapshot_rollback'] = dict(_old_weights)
