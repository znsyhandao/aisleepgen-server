...
def handle_chat(data):
    ...
    # ===== 情绪追踪（记录到 profile） =====
    try:
        from emotion_monitor import record_emotion, get_emotion_trigger
        record_emotion(profile, message)
        # 检测是否需要触发情绪推送（后台标记，不阻塞主流程）
        emotion_trigger = get_emotion_trigger(profile)
        if emotion_trigger and openid != 'default':
            from emotion_monitor import generate_emotion_push_content
            push_content = generate_emotion_push_content(emotion_trigger, profile)
            if push_content:
                # 写入推送队列，稍后由 scheduler 或小程序轮询获取
                from scheduler_daemon import _load_push_queue, _save_push_queue
                import threading
                now = time.time()
                entry = {
                    'id': f'{openid}_emo_{int(now)}',
                    'openid': openid,
                    'title': push_content[0],
                    'content': push_content[1],
                    'push_type': push_content[2],
                    'strategy': 'emotion_monitor',
                    'reason': emotion_trigger['trigger_type'],
                    'pushed_at': now,
                    'expires_at': now + 86400 * 2,
                    'sent': False,
                    'read': False,
                }
                with scheduler_daemon.PUSH_LOCK if hasattr(scheduler_daemon, 'PUSH_LOCK') else threading.Lock():
                    q = _load_push_queue()
                    q.append(entry)
                    _save_push_queue(q)
                _ai_log.info('[Emotion] Push queued for %s: %s', openid[:8], emotion_trigger['trigger_type'])
    except Exception as e:
...
def handle_subscribe_msg(data):
    ...
    if openid != 'default' and tmpl_ids:
        def modifier(p):
            ...
            return p
        _px._atomic_write_profile(openid, modifier)
        _ai_log.info('[Subscribe] %s subscribed to %s (%d templates)',
