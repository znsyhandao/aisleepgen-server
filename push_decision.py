...
def _load_push_threshold(default=50, openid=None):
    """从元学习params读推送阈值（支持用户级覆盖）"""
    import json, os
    try:
        ...
        if openid:
            ...
            if os.path.exists(upath):
                with open(upath, 'r', encoding='utf-8') as f:
                    d = json.load(f)
                if 'push_threshold' in d:
                    return d['push_threshold']
        p = os.path.join(base, 'data', 'params.json')
        if os.path.exists(p):
            with open(p, 'r', encoding='utf-8') as f:
                d = json.load(f)
            return d.get('push_threshold', default)
    except Exception as _e:
        _log = logging.getLogger('push_decision')
        _log.warning('get_threshold failed: %s', _e)
    return default
...
def decide_interaction(openid, event_type, event_data, profile=None):
    ...
    Returns:
        dict: {
            ...
            'detail': str,            # 决策理由（日志用）
            'simulation_info': dict or None,  # 内感受预测结果
    ...
    try:
        if openid != 'default':
            ...
            sim_info = simulate_push_effect(profile)
    except ImportError:
        pass
    except Exception:
        pass

    # 如果仿真说推送效果会很差，压制"推"类动作
    _suppress_push = sim_info and sim_info.get('expected_engagement') == 'negative' and sim_info.get('should_push') is False
    ...
    if event_type == 'chat_emotion':
        return _decide_chat_emotion(openid, event_data, profile, should_skip, sim_info=sim_info)

    elif event_type == 'score_update':
        return _decide_score_update(openid, event_data, profile, should_skip, sim_info=sim_info)

    elif event_type == 'inactive':
        return _decide_inactive(openid, event_data, profile, sim_info=sim_info)

    elif event_type == 'periodic_scan':
        return _decide_periodic(openid, profile, should_skip, sim_info=sim_info)

    else:
        return {'action': 'skip', 'content': '', 'title': None, 'detail': f'unknown event: {event_type}'}


def _decide_chat_emotion(openid, event_data, profile, should_skip, sim_info=None):
    """聊天情绪事件决策
...
def _decide_score_update(openid, event_data, profile, should_skip, sim_info=None):
    """评分更新事件决策
    ...
    # ===== 内感受预测：仿真推送的反效果 =====
    _sim_suppress = sim_info and sim_info.get('expected_engagement') == 'negative' and sim_info.get('should_push') is False
    if _sim_suppress:
        # 仿真说明用户对推送反应差 → 改为安静陪伴（也不延后）
        _record_intervention_log(openid, 'simulation_suppressed', f'score={score}, push_bad_reaction')
        return {
            'action': 'skip',
            'content': '',
            'title': None,
            'detail': f'suppressed by simulation: push had negative effect on this user',
    ...
    if score < 50 and has_emotion_issue:
        ...
        # 评分极低 → 立即推送（仿真若禁止，降级为延迟）
        if _sim_suppress:
            # 即使低评分，仿真说推送效果差 → 改为延迟到早上
            title = f'💤 {username}，睡眠需要关注'
            content = f'昨晚评分{score}分，明显偏低。早上起来回顾一下，今晚试试更早放松。'
            _record_intervention_log(openid, 'score_alert_delayed_by_sim', f'score={score}')
            return {
                'action': 'delay_push',
                'content': content,
                'title': title,
                'detail': f'very low score={score}, delayed by simulation (negative push history)',
                'delay_hours': _get_delay_to_next_slot('morning'),
                'simulation_info': sim_info,
            }
    ...
    return {'action': 'skip', 'content': '', 'title': None, 'detail': 'no strong trigger', 'simulation_info': sim_info}


def _decide_inactive(openid, event_data, profile, sim_info=None):
    """不活跃检测事件决策
...
def _decide_periodic(openid, profile, should_skip, sim_info=None):
    """定时扫描决策（scheduler 调用）
