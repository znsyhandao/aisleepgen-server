...
for i, chunk in enumerate(gen):
    print(f'chunk {i}: {type(chunk)} shape={chunk.shape if hasattr(chunk, \"shape\") else len(chunk)}')
    audio_list.append(chunk)

print(f'Total chunks: {len(audio_list)}')
if audio_list:
    import numpy as np
    flat = np.concatenate([np.asarray(a).flatten() for a in audio_list])
    sf.write('output_reborn/caizhi_600epoch.wav', flat, 32000)
    print(f'OK: {len(flat)/32000:.1f}s')
else:
    print('Failed: empty')
"
"""
