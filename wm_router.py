...
# -*- coding: utf-8 -*-
"""
wm_router.py — 可学习的上下文路由网络 v2.0 (Toto 2.0 集成)

v2.0 新增: Toto2FeatureExtractor — 用预训练时序基础模型替代纯 numpy MLP。
当配置了 TOTO_MODEL_PATH 时，将用户时序数据通过 Toto 2.0 编码为特征向量，
替代原来的 3 层 MLP 手写特征。

架构:
  Input: 16维特征向量 + 可选时序数据 (n_days x n_features)
  → 旧路径: 16维→MLP→3输出 (兼容, 163参数)
  → 新路径: 时序→Toto2 (4M参数)→3输出
  通过模型路径是否存在自动选择路径。
"""
...
# ===== Toto 2.0 配置 =====
TOTO_CACHE_DIR = os.path.join(PROJECT_ROOT, '.toto_cache')
TOTO_MODEL_PATH = os.path.join(TOTO_CACHE_DIR, 'model.safetensors')
TOTO_CFG_PATH = os.path.join(TOTO_CACHE_DIR, 'model_files', 'config.json')
TOTO_CODE_DIR = os.path.join(TOTO_CACHE_DIR, 'toto2')
_TOTO_MODEL = None  # 懒加载单例
...
class WMRouter:
    ...
    def forward(self, features):
        ...
        for feat_dim_raw, biases in fb.items():
            feat_dim = int(feat_dim_raw)  # JSON 会把 key 序列化为 str
    ...
    def predict(self, fields, raw_text, features_seq=None):
        """对外接口：输入neural字段+原始文本 → 检索策略

        Args:
            fields: neural_extractor提取字段
            raw_text: 用户原始输入
            features_seq: list[list[float]], 可选历史时序特征序列，传给Toto2增强
        ...
        # Toto 2.0 增强路径：如果有历史时序数据，增强特征向量
        if features_seq is not None:
            toto_features = toto2_encode(features_seq)
            if toto_features is not None and len(toto_features) == len(features):
                # 加权融合：保留单次特征的70% + Toto时序特征的30%
                features = [
                    0.7 * f + 0.3 * t
                    for f, t in zip(features, toto_features)
                ]
                _ai_log.debug('[Toto2] 特征增强成功')
...
def load_toto2_model():
    """懒加载 Toto 2.0 模型（单例）"""
    global _TOTO_MODEL
    if _TOTO_MODEL is not None:
        return _TOTO_MODEL
    if not os.path.exists(TOTO_MODEL_PATH) or not os.path.exists(TOTO_CODE_DIR):
        _ai_log.warning('[Toto2] 模型文件不存在，回退旧MLP')
        return None
    try:
        import sys as _sys
        _sys.path.insert(0, TOTO_CACHE_DIR)
        from toto2.model import Toto2Model
        import json as _json
        with open(TOTO_CFG_PATH) as _f:
            _cfg_dict = _json.load(_f)
        _cfg = type('Cfg', (), _cfg_dict)()
        _model = Toto2Model(_cfg)
        from safetensors.torch import load_file as _sf
        _state = _sf(TOTO_MODEL_PATH)
        _model.load_state_dict(_state, strict=False)
        _model.eval()
        _TOTO_MODEL = _model
        _ai_log.info('[Toto2] 加载成功: 4.1M参数, CPU推理')
        return _model
    except Exception as _e:
        _ai_log.warning('[Toto2] 加载失败: %s', _e)
        return None


def toto2_encode(features_seq):
    """将用户时序特征通过 Toto2 编码为增强特征向量

    Args:
        features_seq: list[list[float]], 每个元素是16维特征向量

    Returns:
        list[float] | None: 16维增强特征，失败返回 None
    """
    model = load_toto2_model()
    if model is None:
        return None
    if not features_seq or len(features_seq) < 2:
        return None
    try:
        # 构造输入: (1, n_variates, time_steps)
        n_variates = len(features_seq[0])
        time_steps = len(features_seq)
        # 补齐到 patch_size 的整数倍
        patch_size = 32
        pad = (patch_size - time_steps % patch_size) % patch_size
        if pad > 0:
            features_seq = features_seq + [features_seq[-1]] * pad
            time_steps = len(features_seq)

        tensor = torch.tensor(features_seq, dtype=torch.float32).unsqueeze(0).transpose(1, 2)
        mask = torch.ones(1, n_variates, time_steps, dtype=torch.bool)
        ...
        with torch.no_grad():
            out = model(
                ...
                target_mask=mask,
                cpm_mask=mask,
                series_ids=torch.zeros(1, n_variates, dtype=torch.long),
            )
        # 从输出分布中提取 loc 作为特征
        loc = out.loc  # (1, n_variates, time_steps)
        # 取最后一个时间步作为特征向量
        last_step = loc[0, :, -1].tolist()
        return last_step
    except Exception as _e:
        _ai_log.warning('[Toto2] 编码失败: %s', _e)
        return None
