...
def handle_chat(data):
    ...
    # 先查 DeepSeek 缓存
    cached_reply = cache_layer.get_ds_cache(openid, message)
    if cached_reply:
        reply = cached_reply
        token_estimate = len(reply) // 4
        _ai_log.info('[Cache] DS reply reused for %s (%d chars)', openid[:8], len(reply))
        ...
        # 写入缓存
        if api_success and reply:
            cache_layer.set_ds_cache(openid, message, reply)
