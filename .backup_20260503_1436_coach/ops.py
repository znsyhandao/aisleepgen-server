...
# ===== 版本号 =====
VERSION = '2.7.0'
VERSION_TAG = 'cache-layer'  # 推理缓存层：世界模型+API+Profile三缓存
CHANGES = [
    'v2.7.0 Reasoning cache layer (cache_layer.py) — three-level caching',
    'v2.7.0 World model cache: reuse for repeated queries',
    'v2.7.0 DeepSeek API cache: 2h TTL user+query pairs',
    'v2.7.0 Profile memory cache: write-behind, no disk I/O per request',
    'v2.7.0 Background flush daemon: auto-persists dirty profiles every 5s',
    'v2.7.0 Expected latency: 15-30s -> ~1-3s on cache hit',
    'v2.6.0 Chat auto-detect: user says "睡不着" triggers companion (not chatbot reply)',
    'v2.6.0 New API: /api/companion/start, /api/companion/update, /api/companion/status, /api/companion/stop',
    'v2.6.0 Movement relapse handling: up to 5 resets with adaptive backoff',
    'v2.6.0 Safe predefined protocols: 4-7-8 breathing, body scan, gentle breathing',
    'v2.6.0 Fade-out on quiet detection: 30s no movement → gradual exit',
    'v2.3.0 WeChat service notification push engine (wechat_push.py)',
    'v2.3.0 Smart push content generation (morning recap / evening care / alerts)',
    'v2.3.0 Scheduler daemon v2: saves push queue AND sends WeChat pushes',
    'v2.3.0 New API: /api/subscribe-msg, /api/push-settings, /api/pending-push (v2)',
    'v2.3.0 Frontend: profile page subscribe button + push notification list',
