...
# ===== 版本号 =====
VERSION = '2.8.0'
VERSION_TAG = 'sleep-coach'  # sleep coach: suggestion->action->feedback->optimize loop  # 推理缓存层：世界模型+API+Profile三缓存
CHANGES = [
    'v2.8.0 sleep_coach.py - sleep improvement coach: suggestion->action->feedback->optimize loop',
    'v2.8.0 Daily 1 actionable suggestion auto-generated per user',
    'v2.8.0 Yesterday suggestion effect evaluation with score change tracking',
    'v2.8.0 Adaptive strategy: effective suggestions boosted, weak ones phased out',
    'v2.8.0 Coach reminders via push_decision at preset time slots',
    'v2.8.0 Suggestion embedded in chat reply (no separate push needed)',
]
...
VERSION_TAG = 'sleep-coach'  # sleep coach: suggestion->action->feedback->optimize loop  # 睡眠教练：建议→执行→反馈→优化闭环
