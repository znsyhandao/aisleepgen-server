"""
audio_pomdp_bridge.py — 音频分析结果 → POMDP观测桥接 + 呼吸引导推荐

v2: 使用sleep_audio_analyzer v3的可配置手环接口
"""
import os, json
from sleep_audio_analyzer import SleepAudioAnalyzer, get_analyzer, SLEEP_RECORD_DIR

AUDIO_OBS_JSON = os.path.join(SLEEP_RECORD_DIR, "audio_analysis_results.json")

def get_latest_audio_observation(openid: str = "default", ring_provider=None) -> dict:
    """
    读取最近的音频分析结果，转换为POMDP观测格式
    ring_provider: 可选手环数据提供函数
    """
    ana = get_analyzer(ring_provider=ring_provider)
    ...
    obs = ana.audio_to_pomdp_observation(wav_results)
    ...
    # 获取呼吸引导推荐
    breathing = ana.suggest_breathing_pattern(wav_results)
    
    # 学习趋势
    ana.learn_from_history(wav_results)
