...
# ===== 版本号 =====
VERSION = '2.11.0'
VERSION_TAG = 'expert-viz'  # 10-expert panel visualization in chat: transparent AI reasoning  # fast local reply + background deep analysis: user wait 6ms not 5s  # user feedback loop: close the coach suggestion->action->feedback loop  # sleep coach: suggestion->action->feedback->optimize loop  # 推理缓存层：世界模型+API+Profile三缓存
CHANGES = [
    'v2.11.0 Expert panel visualization in chat bubbles (10 experts)',
    'v2.11.0 Chat wxml+js: collapsible expert detail card per AI reply',
    'v2.11.0 Expert data: name, specialty, score, findings per expert',
    'v2.11.0 Score colors: 优秀(green) 良好(light green) 一般(orange) 偏低(red)',
    'v2.11.0 Backend: expert_detail field from world model passed to frontend',
    'v2.11.0 Async pipeline: passes wm_result expert_detail to dp_router',
]
...
VERSION_TAG = 'expert-viz'  # 10-expert panel visualization in chat: transparent AI reasoning  # fast local reply + background deep analysis: user wait 6ms not 5s  # user feedback loop: close the coach suggestion->action->feedback loop  # sleep coach: suggestion->action->feedback->optimize loop  # 睡眠教练：建议→执行→反馈→优化闭环
