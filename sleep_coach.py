...
def _select_suggestion(profile, history, avg_score, trend, emotion_state, pomdp_weights=None):
    """选择最合适的改善建议（已增强：仿真引擎优先，POMDP信念加权）
    ...
    Args:
        ...
        trend: 趋势
        emotion_state: 情绪状态
        pomdp_weights: v4.2 可选的8维POMDP信念权重 [s0_weight, ..., s7_weight]
                       如果提供，按状态-建议映射表对候选分数加权
    """
    coach = profile.get('sleep_coach', {})
    ...
    # v4.2: 状态-建议偏好映射（每状态对每种建议的偏好权重）
    # 8个状态（索引0-7）：acute_insomnia, chronic_poor, relapse,
    #                      stable_good, stable_moderate, anxiety_driven,
    #                      circadian_drift, recovering
    STATE_SUG_PREF = {
        'routine':  [0.05, 0.30, 0.05, 0.05, 0.20, 0.05, 0.35, 0.20],
        'wind_down': [0.35, 0.20, 0.30, 0.05, 0.10, 0.25, 0.05, 0.25],
        'no_screen': [0.10, 0.15, 0.10, 0.10, 0.15, 0.05, 0.10, 0.10],
        'walk':      [0.05, 0.05, 0.05, 0.20, 0.10, 0.05, 0.05, 0.05],
        'earlier':   [0.10, 0.05, 0.05, 0.05, 0.10, 0.05, 0.25, 0.05],
        'journal':   [0.15, 0.10, 0.20, 0.05, 0.05, 0.30, 0.05, 0.10],
        'hydrate':   [0.05, 0.05, 0.05, 0.10, 0.10, 0.05, 0.05, 0.05],
        'temp':      [0.05, 0.05, 0.05, 0.10, 0.05, 0.05, 0.05, 0.05],
        'stretch':   [0.05, 0.05, 0.05, 0.10, 0.10, 0.05, 0.05, 0.05],
        'breath':    [0.05, 0.10, 0.10, 0.05, 0.05, 0.10, 0.05, 0.10],
    }
    ...
    # ===== v4.2: POMDP 8态信念驱动建议选择 =====
    # 用8个语义状态的概率加权建议评分
    # 急性失眠→wind_down加权，慢性差→routine加权，焦虑→journal加权等
    if pomdp_weights is not None and len(pomdp_weights) == 8:
        weighted_candidates = []
        for key, sug, base in candidates:
            # 查询状态-建议偏好
            prefs = STATE_SUG_PREF.get(key, [0.125]*8)
            # 内积：pomdp_weights · state_pref 作为乘数
            multiplier = sum(pomdp_weights[s] * prefs[s] for s in range(8))
            # 归一化到 [0.5, 2.0] 范围
            norm_mult = 0.5 + multiplier * 3.0
            weighted_candidates.append((key, sug, base * norm_mult))
        if weighted_candidates:
            candidates = weighted_candidates
    else:
        # 降级：旧的启发式bonus（保留作为fallback）
        try:
            ...
            if openid:
                ...
                exp_score = belief.get('expected_score', 50)
                ent = belief.get('normalized_entropy', 1.0)
                if ent < 0.95:
                    pomdp_candidates = []
                    for key, sug, base in candidates:
                        bonus = 0
                        cat = sug.get('category', '')
                        if 'stress' in cat and exp_score < 50:
                            bonus = 20
                        elif 'habit' in cat and 50 <= exp_score <= 70:
                            bonus = 15
                        elif 'relax' in cat and ent > 0.6:
                            bonus = 10
                        elif 'education' in cat and ent < 0.3:
                            bonus = 5
                        pomdp_candidates.append((key, sug, base + bonus))
                    if pomdp_candidates:
                        candidates = pomdp_candidates
        except ImportError:
            pass
        except Exception:
            pass

    # ===== 内感受预测增强：用仿真引擎排序（代替纯加权随机） =====
    try:
        from interoceptive_prediction import select_suggestion_with_simulation
        sim_result = select_suggestion_with_simulation(profile, candidates, emotion_state)
        if sim_result and sim_result[0]:
            return sim_result
    except ImportError:
        pass  # 无仿真引擎时走旧路径
    except Exception as e:
        _log.warning('[Coach] Simulation failed, using legacy: %s', e)
...
def get_daily_suggestion(profile, emotion_state='neutral'):
    ...
    pomdp_weights = None
    try:
        from pomdp_learner import get_engine as _ge
        _pe = _ge()
        openid = profile.get('openid', '')
        if openid:
            _bel = _pe.get_belief(openid)
            bp = _bel.get('belief_probs', [])
            if len(bp) == 8:
                pomdp_weights = bp
    except Exception:
        pass

    key, sug = _select_suggestion(profile, history, avg_score, trend, emotion_state, pomdp_weights=pomdp_weights)
    ...
    # v3.0: 昼夜节律感知增强——注入时间感知上下文
    circ_context = ''
    try:
        from circadian_phase_model import get_drowsiness_forecast
        forecast = get_drowsiness_forecast(profile.get('openid', 'default'))
        if forecast.get('available'):
            params = forecast.get('parameters', {})
            drift = params.get('drift_rate', 0)
            window = params.get('optimal_window', None)
            amp = params.get('amplitude', 10)

            # 如果生物钟在漂移，在建议中附加节律提醒
            if drift > 10:
                circ_context = ' ⚠️ 近期入睡时间持续后移'
                action_text += f'（近几天你睡得越来越晚，尽量固定在目标时间）'
            elif drift > 5:
                circ_context = ' 📊 入睡时间略有后移'

            # 如果用户当前在最佳就寝窗口内，强化"现在睡"的感知
            if window:
                now_hour = datetime.now().hour + datetime.now().minute / 60.0
                if window[0] <= now_hour <= window[1]:
                    action_text += ' 🌙 现在正好是适合入睡的时间段！'
                elif abs(now_hour - window[0]) < 1:
                    mins_left = int((window[0] - now_hour) * 60)
                    if mins_left > 0:
                        action_text += f' ⏰ 你的生物钟即将进入易睡窗口（约{mins_left}分钟后）'
    except ImportError:
        pass
    except Exception as e:
        _log.warning('[Coach] Circadian enhancement failed: %s', e)

    return {
        'suggestion_key': key,
        'title': sug['name'],
        'action': action_text,
        'circadian_context': circ_context,
...
def record_feedback(profile, suggestion_key, feedback, date=None):
    """Record user feedback on a coach suggestion"""
    coach = profile.setdefault('sleep_coach', {})
    target_date = date or datetime.now().strftime('%Y-%m-%d')

    effect = 0
    for h in coach.get('history', []):
        if h.get('date') == target_date and h.get('suggestion') == suggestion_key:
            h['user_feedback'] = feedback
            h['feedback_at'] = datetime.now().isoformat()
            if feedback == 'done':
                h['completed'] = True
                effect = 15
            elif feedback == 'not_done':
                effect = -10
    ...
    effectiveness = coach.setdefault('effectiveness', {})
    old = effectiveness.get(suggestion_key, 50)
    new_val = max(5, min(100, old + effect))
    effectiveness[suggestion_key] = round(new_val, 1)
    return profile, {'suggestion': suggestion_key, 'feedback': feedback, 'effect_change': effect}


def get_pending_feedback(profile):
    """Get suggestions awaiting user feedback"""
    coach = profile.get('sleep_coach', {})
    history = coach.get('history', [])
    ...
    for h in reversed(history):
        if isinstance(h, dict):
            d = h.get('date', '')
            if d == yesterday and not h.get('user_feedback'):
                return {'suggestion_key': h['suggestion'], 'title': h.get('title',''), 'action': h.get('action',''), 'date': d}
            if d == today and not h.get('user_feedback') and coach.get('given_today', False):
                return {'suggestion_key': h['suggestion'], 'title': h.get('title',''), 'action': h.get('action',''), 'date': d}
    return None


def get_feedback_stats(profile):
    """Get feedback statistics"""
    coach = profile.get('sleep_coach', {})
    history = coach.get('history', [])
    total = len(history)
    feedback_count = sum(1 for h in history if isinstance(h, dict) and h.get('user_feedback'))
    done_count = sum(1 for h in history if isinstance(h, dict) and h.get('user_feedback') == 'done')
    return {
        'total': total,
        'feedback': feedback_count,
        'done': done_count,
        'completion_rate': round(done_count / total * 100, 1) if total > 0 else 0,
        'feedback_rate': round(feedback_count / total * 100, 1) if total > 0 else 0,
    }
