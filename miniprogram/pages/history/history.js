...
Page({
  ...
  loadHistoryRecords() {
    ...
    // 本周日历
    this._updateWeekCalendar(filteredRecords);
  },
  ...
  // 本周日历颜色
  _scoreColor(score) {
    if (score >= 85) return 'rgba(76, 175, 80, 0.4)';
    if (score >= 75) return 'rgba(139, 195, 74, 0.35)';
    if (score >= 65) return 'rgba(255, 193, 7, 0.3)';
    if (score >= 50) return 'rgba(255, 152, 0, 0.3)';
    return 'rgba(244, 67, 54, 0.25)';
  },

  _updateWeekCalendar(allRecords) {
    ...
    for (let i = 6; i >= 0; i--) {
      ...
      const record = allRecords.find(r => {
        ...
        return rDate.toLocaleDateString() === dateStr;
      });
