...
Page({
  ...
  onLoad(options) {
    ...
    // v7.2: 加载诊断书数据
    api.getSiegeDiagnosis().then(function(diagRes)
      {
if (diagRes && diagRes.success && diagRes.diagnosis) {
        var diag = diagRes.diagnosis;
        self.setData({
          ...
          diagnosisCardText: diagRes.card_text || '',
  ...
  _displayResult(result) {
    ...
    // 解析决策内参为可交互卡片
    var memoParsed = null;
    var decisionMemo = result.decisionMemo || '';
    if (decisionMemo) {
      memoParsed = [];
      var parts = decisionMemo.split('|');
      for (var i = 0; i < parts.length; i++) {
        var part = parts[i].trim();
        var colonIdx = part.indexOf(']');
        if (colonIdx >= 0) {
          var label = part.substring(1, colonIdx).trim();
          var content = part.substring(colonIdx + 1).trim();
          var detail = [];
          // 优先使用后端返回的动态详情，无则回退空数组
          var detailData = result.decisionMemoDetail || {};
          for (var key in detailData) {
            if (label.indexOf(key) >= 0) {
              detail = detailData[key];
              break;
            }
          }
          memoParsed.push({
      ...
      if (memoParsed.length === 0) memoParsed = null;
    }
    ...
    this.setData({
      ...
      predictionStats: result.predictionStats || null,
      // [v2026-07-04] 睡眠韧性 + 状态拓扑（后端下划线→前端驼峰）
      resilienceIndex: result.resilience_index ? {
        overall: result.resilience_index.overall,
        level: result.resilience_index.level,
        detail: result.resilience_index.detail,
        dimensions: (function() {
          var dims = [];
          for (var k in result.resilience_index.dimensions) {
            var nameMap = {circadian_resilience:'节律', emotional_resilience:'情绪', physical_resilience:'身体'};
            dims.push({key: k, name: nameMap[k] || k, value: result.resilience_index.dimensions[k]});
          }
          return dims;
        })(),
      } : null,
      stateTopology: result.state_topology || result.stateTopology ? (function() {
        var topo = result.state_topology || result.stateTopology;
        if (!topo || !topo.has_topology) return null;
        // 给recent_nights加trend方向
        var nights = (topo.recent_nights || []).map(function(n, i, arr) {
          var prev = i > 0 ? arr[i-1].distance : n.distance;
          return {
            date: n.date,
            distance: n.distance,
            distance_score: Math.round((1 - Math.min(n.distance, 1)) * 100),
            trend: n.distance > prev ? 'up' : n.distance < prev ? 'down' : 'flat'
          };
        });
        // 吸引子的平均分
        var attrScores = result._attractor_scores || '';
        return {
          has_topology: true,
          current_dscore: topo.current_dscore,
          trend_direction: topo.trend_direction,
          sample_count: topo.sample_count,
          recent_nights: nights,
          attractor_scores: attrScores || '好',
        };
      })() : null,
      loading: false,
      showReport: true,
      ...
      // 付费状态
      paywall: result.paywall || false,
      paywallMessage: result.paywall_message || ''
    });
    
    // 评分动画
    this._animateScore(0, result.score || 75);
    
    // [v2026-06-29] 方案C：客户端缓存缓存key
    if (result._cache_key) {
      wx.setStorageSync('_report_cache_' + result._cache_key, {
        timestamp: Date.now(),
        data: result
      });
    }
  ...
  _loadFromStorage() {
    ...
    // v7.2: 并行加载chart数据
    api.getChartData().then(function(chartRes)
      {
if (chartRes && chartRes.success && chartRes.chart) {
        var chart = chartRes.chart;
    ...
    // [v2026-06-29] 方案C：检查缓存的报告（跳过API调用）
    var cachedReport = wx.getStorageSync('_cached_report');
    if (cachedReport && cachedReport.timestamp && (Date.now() - cachedReport.timestamp < 86400000)) {
      // 24小时内有效
      console.log('[ReportCache] 命中客户端缓存');
      this._displayResult(cachedReport.data);
      return;
    }
    ...
    if (lastResult) {
      ...
      // 异步获取预测验证统计
      api.getPredictionStats().then(function(predRes) {
        if (predRes && predRes.success && predRes.stats) {
          self.setData({ predictionStats: predRes.stats });
        }
      }).catch(function() {});
    } else {
  ...
  toggleAltView() {
    this.setData({ showAltView: !this.data.showAltView });
  },

  toggleMemoDetail(e) {
    var index = e.currentTarget.dataset.index;
    var activeIndex = this.data.activeMemoIndex;
    if (activeIndex === index) {
      this.setData({ activeMemoIndex: -1 });
    } else {
      this.setData({ activeMemoIndex: index });
    }
  },
  // ===== 操作 =====
  viewDetail(e) {
    ...
    const labels = {
      deepSleep: '深睡时长', remSleep: 'REM睡眠', lightSleep: '浅睡+醒时长',
      awakeTime: '清醒时间⚠️', sleepEfficiency: '睡眠效率',
  ...
  goUpgrade() {
    wx.showToast({ title: '正在跳转会员中心', icon: 'none' });
    wx.navigateTo({ url: '/pages/profile/profile?tab=member' });
  },
