...
_env_secret = os.environ.get('HUAWEI_CLIENT_SECRET')
if _env_secret:
    ...
    if os.path.exists(_env_path):
        for _line in open(_env_path, 'r', encoding='utf-8'):
            _line = _line.strip()
            if _line.startswith('HUAWEI_CLIENT_SECRET='):
                CLIENT_SECRET = _line.split('=', 1)[1].strip().strip("'\"")
                break
    else:
        CLIENT_SECRET = ""  # 留空会在运行时报错，引导用户配置
REDIRECT_URI = "http://123.60.222.129:8090/api/huawei/callback"
AUTH_STATE = "aisleepgen_v1"

# 生产环境回调（部署时替换）
PROD_REDIRECT_URI = "https://your-domain.com/api/huawei/callback"
...
def get_miniprogram_auth_url() -> dict:
    """小程序专用：生成授权URL（用webview打开）"""
    url = get_authorization_url()
    return {
        "url": url,
        "redirect_uri": REDIRECT_URI,
        "tip": "打开此链接后，使用华为账号登录并授权Health Kit读取睡眠数据",
    }
...
def _get_token_path(openid: str) -> str:
    """获取指定用户的token文件路径"""
    return os.path.join(PROJECT_ROOT, "data", f"huawei_token_{openid}.json")


def sync_for_openid(openid: str) -> dict:
    """为指定用户拉取手环数据并注入
    
    这是cron和小程序调用的统一入口
    """
    # 使用用户专属的token路径
    token_path = _get_token_path(openid)
    if not os.path.exists(token_path):
        return {"status": "need_auth", "message": "该用户未授权华为手环"}
    
    # 加载用户token
    tm = TokenManager(token_path)
    saved = tm.load()
    if not saved:
        return {"status": "need_auth", "message": "token不存在"}
    
    # 获取有效token（自动刷新）
    token = saved.get("access_token", "")
    if not token:
        return {"status": "need_auth", "message": "token无效"}
    
    # 拉取数据
    try:
        sleep_data = fetch_sleep_data(token)
        hr_data = fetch_heart_rate(token)
        hrv_data = fetch_hrv(token)
        
        obs = format_sleep_for_pomdp(sleep_data, hr_data, hrv_data)
        if not obs.get("available"):
            return {"status": "no_data", "message": "暂无睡眠数据"}
        ...
        try:
            ...
            # 同时写入profile供前端展示
            try:
                from utils import _load_user_profile, _save_user_profile
                profile = _load_user_profile(openid)
                profile.setdefault('devices', {}).setdefault('huawei_band', {})
                profile['devices']['huawei_band']['synced_at'] = datetime.now().isoformat()
                profile['devices']['huawei_band']['last_sleep_data'] = {
                    'total_min': obs['raw']['total_sleep_min'],
                    'deep_min': obs['raw']['deep_sleep_min'],
                    'hr_avg': obs['raw']['heart_rate_avg'],
                    'hrv_avg': obs['raw']['hrv_avg'],
                }
                _save_user_profile(openid, profile)
            except Exception as e:
                print(f"[Huawei] profile update failed: {e}")
...
def sync_all_users() -> dict:
    """cron入口：为所有已授权的用户拉取手环数据"""
    results = {"total": 0, "success": 0, "skipped_no_data": 0, "errors": 0, "details": []}
    ...
    for fname in os.listdir(data_dir):
        if not fname.startswith("huawei_token_") or not fname.endswith(".json"):
            continue
        openid = fname.replace("huawei_token_", "").replace(".json", "")
        results["total"] += 1
        try:
            r = sync_for_openid(openid)
            if r.get("status") == "injected":
                ...
                results["errors"] += 1
            results["details"].append({"openid": openid[:12], "status": r.get("status")})
        except Exception as e:
            results["errors"] += 1
            results["details"].append({"openid": openid[:12], "status": "error: " + str(e)[:40]})
    
    print(f"[Huawei] sync_all: {results['total']} users, {results['success']} success, {results['skipped_no_data']} no_data, {results['errors']} errors")
    return results
    print()
